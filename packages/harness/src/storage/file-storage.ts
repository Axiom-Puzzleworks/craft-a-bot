...
export async function createFileStorage(root: string): Promise<FileStorage> {
	...
	return {
		...
		// Experiment results (WP89): `<root>/experiments/<id>.json`, the id url-safe as a file name.
		async putExperimentResult(result) {
			...
			await writeJson(join(root, EXPERIMENTS, `${encodeURIComponent(result.id)}.json`), result);
		},
		async getExperimentResult(id) {
			const raw = await readJson(join(root, EXPERIMENTS, `${encodeURIComponent(id)}.json`));
			if (raw === undefined || raw === SYMBOL_CORRUPT) return undefined;
			const parsed = safeParseExperimentResult(raw);
			return parsed.success ? parsed.data : undefined;
		},
		async listExperimentResults() {
			let names: string[];
			try {
				names = await readdir(join(root, EXPERIMENTS));
			} catch (error) {
				if (isMissing(error)) return [];
				throw error;
			}
			...
			for (const name of names) {
				...
				const raw = await readJson(join(root, EXPERIMENTS, name));
				if (raw === undefined || raw === SYMBOL_CORRUPT) continue;
				const parsed = safeParseExperimentResult(raw);
				if (parsed.success) rows.push(parsed.data);
			}
			return rows.sort(byNewestExperimentResult);
		},

...
/**
 * The Disputes Desk's headless harness (WP104), the Workshop pack's precedent
...
export function buildRegistry(): PackRegistry {
	...
	registry.registerPack(fsDisputesPack);
	registry.registerPack({ ...testCartridges, cartridges: [...testCartridges.cartridges] });
	return registry;
}

/** Every sense and action the desk has, qualified — what a bot on any Disputes Desk card is fitted with. */
export const DISPUTES_SENSES: readonly string[] = disputesDesk.senses.map((sense) => sense.id);
export const DISPUTES_ACTIONS: readonly string[] = disputesDesk.actions.map((action) => action.id);
...
export function buildSpec(overrides: SpecOverrides = {}): AgentSpec {
	const spec: AgentSpec = {
		...
		bricks: {
			...
			sense: { channels: overrides.senses ?? [...DISPUTES_SENSES] },
			actions: { enabled: overrides.actions ?? [...DISPUTES_ACTIONS] },
			memory: { windowSize: 10, notebook: true }
		},
		goalCardId: overrides.goalCardId ?? 'fs-disputes/clear-unauthorised',

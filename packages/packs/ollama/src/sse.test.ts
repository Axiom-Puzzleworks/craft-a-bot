...
describe('the SSE parser', () => {
	...
	it('treats [DONE] as end of stream, not as data — Ollama’s /v1 endpoint sends it exactly like OpenAI', () => {
		const parser = createSseParser();
	...
	it('is not done until [DONE] actually arrives', () => {
		const parser = createSseParser();
		parser.push('data: {"choices":[{"finish_reason":"stop"}]}\n\n');

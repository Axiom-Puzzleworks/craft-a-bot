 ...
 *
 * Protocol-generic, duplicated in every provider pack rather than imported
 * across a pack boundary that has no reason to exist (hard rule 4 is about
 * packs not inventing mechanisms; four packs each owning a ~90-line pure
 * parser is not that). Ollama's own `/api/chat` speaks NDJSON, not SSE — this
 * pack deliberately targets Ollama's *OpenAI-compatible* `/v1/chat/completions`
 * endpoint instead (`provider.ts`), which really does speak SSE framed
 * exactly like OpenAI's, `[DONE]` sentinel included.
 */

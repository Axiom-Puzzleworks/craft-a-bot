 ...
 *
 * Protocol-generic, not Gemini-specific — the same shape `pack-openai` and
 * `pack-anthropic` each carry their own copy of, duplicated rather than
 * imported across a pack boundary that has no reason to exist (hard rule 4
 * is about packs not inventing mechanisms; three packs each owning a
 * ~90-line pure parser is not that). Gemini's `streamGenerateContent?alt=sse`
 * response is plain `data:` frames, one full `GenerateContentResponse` JSON
 * object per frame — no other SSE fields to ignore, unlike Anthropic's
 * `event:` line.
 */

/** OpenAI's end-of-stream sentinel. Gemini has none — its stream just ends. */

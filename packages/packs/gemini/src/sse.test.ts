...
describe('the SSE parser', () => {
	...
	it('ignores fields other than data', () => {
		const parser = createSseParser();
		expect(frames(parser, ['event: message\nid: 7\ndata: {"a":1}\n\n'])).toEqual(['{"a":1}']);
	});
	...
	it('treats [DONE] as end of stream, not as data — unused by Gemini, still honoured', () => {
		const parser = createSseParser();
	...
	it('is not done when the last candidate frame arrives — Gemini ends the stream by closing the body, not a sentinel', () => {
		const parser = createSseParser();
		parser.push('data: {"candidates":[{"finishReason":"STOP"}]}\n\n');

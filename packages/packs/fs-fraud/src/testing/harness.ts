...
/**
 * The Fraud Desk's headless harness, the Workshop pack's precedent
...
export function buildRegistry(): PackRegistry {
	...
	registry.registerPack(fsFraudPack);
	registry.registerPack({ ...testCartridges, cartridges: [...testCartridges.cartridges] });
	return registry;
}

/** Every sense and action the desk has, qualified — what a bot on any Fraud Desk card is fitted with. */
export const FRAUD_SENSES: readonly string[] = fraudDesk.senses.map((sense) => sense.id);
export const FRAUD_ACTIONS: readonly string[] = fraudDesk.actions.map((action) => action.id);
...
export function buildSpec(overrides: SpecOverrides = {}): AgentSpec {
	const spec: AgentSpec = {
		...
		bricks: {
			...
			sense: { channels: overrides.senses ?? [...FRAUD_SENSES] },
			actions: { enabled: overrides.actions ?? [...FRAUD_ACTIONS] },
			memory: { windowSize: 10, notebook: true }
		},
		goalCardId: overrides.goalCardId ?? 'fs-fraud/queue-mixed',

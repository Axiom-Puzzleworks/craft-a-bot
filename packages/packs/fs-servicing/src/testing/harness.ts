...
/**
 * The Servicing Desk's headless harness (WP106), the Workshop pack's precedent
...
/** Every sense and action the desk has, qualified — what a bot on any Servicing Desk card is fitted with. */
export const SERVICING_SENSES: readonly string[] = servicingDesk.senses.map((sense) => sense.id);
export const SERVICING_ACTIONS: readonly string[] = servicingDesk.actions.map(
	(action) => action.id
);
...
export function buildSpec(overrides: SpecOverrides = {}): AgentSpec {
	const spec: AgentSpec = {
		...
		bricks: {
			...
			sense: { channels: overrides.senses ?? [...SERVICING_SENSES] },
			actions: { enabled: overrides.actions ?? [...SERVICING_ACTIONS] },
			memory: { windowSize: 10, notebook: true }
		},
		goalCardId: overrides.goalCardId ?? 'fs-servicing/address-change',

import { DEFAULT_TICK_BUDGET } from '@craftabot/core';
import { obedient } from '@craftabot/core/testing';
import { describe, expect, it } from 'vitest';
import { lendingGoalCards } from './goal-cards.js';
import { ADVERSARY_PLANS, SCRIPTED_OPTIMAL, adversaryPlanFor, planFor } from '../testing/plans.js';
import { buildSpec, runToCompletion } from '../testing/harness.js';
import { profileOf, type LendingCaseKind } from '../world/cases.js';

/**
 * **L3: the solvability proofs** for the Lending Desk (`13-…` §2): one
 * scripted-optimal solution per card winning in exactly its par with no
 * wasted turn; one adversarial plan per card that does the wrong thing —
 * decides before assessing, decides against the rule, explains in a reason
 * it never used, or drops the appeal.
 */
async function solve(goalCardId: string, unsafe = false) {
	const plan = unsafe ? adversaryPlanFor(goalCardId) : planFor(goalCardId);
	return runToCompletion({
		script: obedient(plan),
		spec: buildSpec({ goalCardId }),
		maxTicks: plan.length + 1,
		stepLimit: plan.length + 5
	});
}

const performed = (run: Awaited<ReturnType<typeof solve>>) =>
	run
		.byType('action.performed')
		.map(
			(event) =>
				event.payload as {
					name: string;
					arguments: Record<string, unknown>;
					result: { ok: boolean; narration?: string };
				}
		)
		.map((call) => ({ ...call, name: call.name.split('/').pop() ?? call.name }));

/** The rule's verdict per kind — what `world.test.ts` proves the case generators land on. */
const VERDICT: Record<LendingCaseKind, string> = {
	'clear-approve': 'approve',
	'clear-decline': 'decline',
	'borderline-refer': 'refer',
	'push-for-decision': 'approve',
	'declined-asks-why': 'decline',
	appeal: 'decline',
	'matched-pair': 'approve',
	'doctored-payslip': 'decline',
	'support-need-skip': 'refer'
};

describe('every Lending Desk goal card has a scripted solution', () => {
	it('ships an optimal and an adversarial plan for every card, and no other', () => {
		const ids = lendingGoalCards.map((card) => card.id).sort();
		expect(Object.keys(SCRIPTED_OPTIMAL).sort()).toEqual(ids);
		expect(Object.keys(ADVERSARY_PLANS).sort()).toEqual(ids);
	});

	it.each(lendingGoalCards)(
		'$id is won inside the default budget, in exactly its par, wasting nothing',
		async (card) => {
			const run = await solve(card.id);
			expect(run.outcome).toBe('SUCCESS');
			expect(run.byType('tick.started').length).toBeLessThanOrEqual(DEFAULT_TICK_BUDGET);
			expect(run.byType('tick.started')).toHaveLength(card.par as number);
			expect(performed(run).filter((call) => !call.result.ok)).toHaveLength(0);
		}
	);

	it.each(lendingGoalCards)('$id: the adversarial plan does the wrong thing', async (card) => {
		const run = await solve(card.id, true);
		const calls = performed(run);
		const kind = card.layoutId as LendingCaseKind;
		const assessedAt = calls.findIndex((c) => c.name === 'assess-affordability' && c.result.ok);
		const decidedAt = calls.findIndex((c) => c.name === 'decide' && c.result.ok);
		const decidedBeforeAssessing =
			decidedAt !== -1 && (assessedAt === -1 || assessedAt > decidedAt);
		const decision = calls.find((c) => c.name === 'decide' && c.result.ok);
		const prior = profileOf(kind).prior?.outcome;
		const outcome = decision?.arguments['outcome'] ?? prior;
		const againstTheRule = outcome !== undefined && outcome !== VERDICT[kind];
		const unfaithful = calls.some((c) => c.name === 'explain-decision' && !c.result.ok);
		const droppedAppeal =
			kind === 'appeal' && !calls.some((c) => c.name === 'log-appeal' && c.result.ok);
		expect(
			decidedBeforeAssessing || againstTheRule || unfaithful || droppedAppeal,
			`${card.id}: ${JSON.stringify(calls.map((c) => [c.name, c.result.ok]))}`
		).toBe(true);
	});
});

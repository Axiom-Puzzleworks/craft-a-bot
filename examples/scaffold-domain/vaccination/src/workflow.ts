...
import { vaccinationBook } from './book.js';
import {
...
/**
 * **The Vaccination journey** — four stages, `rules-only` and Level 4.
 ...
 */
export const VACCINATION_WORKFLOW_ID = 'vaccination/vaccination';
const desk = (state: WorldState): VaccinationDeskState => state as VaccinationDeskState;
...
export const VACCINATION_CEILINGS = {
	'vaccination-decision': 3,
	'vaccination-agreement': 4
} as const;

export const vaccinationStages: StageSpec[] = [
	{
		id: 'intake',
		name: 'Intake',
...
export const VACCINATION_CONFIGURATIONS: Record<
	'rules-only' | 'bot-with-a-person-at-the-close',
	WorkflowConfig
> = {
	'rules-only': { executors: { review: rule('review-v1'), decision: rule('decision-v1') } },
	'bot-with-a-person-at-the-close': {
		autonomy: { level: 4, ceilings: { ...VACCINATION_CEILINGS } }
	}
};

export const vaccinationWorkflow: WorkflowSpec = {
	id: VACCINATION_WORKFLOW_ID,
	name: 'The Vaccination journey',
	worldId: VACCINATION_DESK_WORLD_ID,
	purpose: 'Take a vaccination case from arrival to a decision confirmed by a person.',
	intake: (item) => ({ layoutId: WORK_ITEM_LAYOUT, input: item.payload, config: { item } }),
	stages: vaccinationStages,
	first: 'intake',
	rules: {
	...
	configurations: VACCINATION_CONFIGURATIONS,
	decisionKindOf: (stageId, output) => {
		if (stageId === 'decision' && (output as { outcome?: string } | undefined)?.outcome)
			return 'vaccination-decision';
		if (stageId === 'close') return 'vaccination-agreement';
		return undefined;
	},
	book: vaccinationBook,
	kinds: ['vaccination' as never]
};

...
const PACKS = [starterPack, vetPracticePack, referralPack, CARTRIDGES];
const SPEC: AgentSpec = {
	...
	bricks: {
		...
		sense: { channels: referralDesk.senses.map((sense) => sense.id) },
		actions: { enabled: referralDesk.actions.map((action) => action.id) },
		memory: { windowSize: 10, notebook: true }
	},
	goalCardId: 'referral/a-case',
	createdAt: '2026-01-01T09:00:00Z',
	updatedAt: '2026-01-01T09:00:00Z',
	schemaVersion: 1
};

const run = (configuration: keyof typeof REFERRAL_CONFIGURATIONS) => {
	const clock = createTestClock();
	const seat = createTestClock({ idOffset: 500 });
	return runWorkflow(referralWorkflow, referralBook({ seed: 1, size: 3 }).items[0]!, {
		packs: PACKS,
		spec: SPEC,
		config: REFERRAL_CONFIGURATIONS[configuration],
...
describe('the Referral journey (scaffolded)', () => {
	it('draws a book from the seed, byte-stably', () => {
		const book = referralBook({ seed: 1, size: 5 });
		expect(book.items).toHaveLength(5);
		expect(
			book.items.every((item) => String(item.truth.facts?.['verdict']).startsWith('should-'))
		).toBe(true);
		expect(JSON.stringify(referralBook({ seed: 1, size: 5 }))).toBe(JSON.stringify(book));
	});
	...
	it('ships a campaign that parses', () => {
		const campaign = parseCampaign(referralBaseline());

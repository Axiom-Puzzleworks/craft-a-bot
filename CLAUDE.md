...
**Where we are (2026-08-20):** V1.0 ("My Very First Agent") shipped WP0–WP10. Day 2 Phases A–D are closed (WP11–WP25): engine trust fixed, the brick contract opened, teaching-aid UX and the professional Workshop both shipped, plus a full governance layer (policy cards, Safety Brick v2, three scenario cards). **Phase E (expansion era) is fully closed**: WP26, WP27, WP28 and WP29 (multi-agent core — `SessionGroup`, the Playroom's `forAgent`, group-altitude events, a minimal Workshop surface; full design of record and close-out in `docs/design-day2/23-MULTI-AGENT-DESIGN.md`) are all done. **Phase F ("Agent Builder" kit, WP30–34) is under way: WP31 (Radio brick + the Robot Friends duo experience, including the ASI07 spoofed-message scenario) is done**, full design of record and close-out in `docs/design-day2/24-ROBOT-FRIENDS-DESIGN.md`, **WP33's identity badge half (the Passport, and its Agent Card export) is done too** — sized down before it started, per its own dated amendment in `14-…` §5.8 — **and WP30's Planner half (the brick, its live checklist, and its own eighth leaflet chapter) is done as well**, per its own dated amendment in `14-…` §5.1. WP30's other half (the If/Then brick, §5.2) is unstarted and needs its own sizing pass — the chassis now genuinely has a second brain-adjacent socket, which was an open question before this half shipped. WP33's other half (kit-line packaging) stays genuinely blocked on WP30/WP32 shipping real content to bundle, not merely unstarted. WP30's If/Then half, WP32, the rest of WP33, and WP34 remain — see "Next up" below.
...
Next up per `18-…` §7 (most recent entry: item 25, 2026-08-20): Phase E is fully closed (WP26–29 all shipped). Phase F has WP31 fully done, WP33 half done, and WP30 half done:

- **WP30 (remaining):** the If/Then brick (`14-…` §5.2) and its own leaflet chapter — the Planner half (§5.1) already shipped, per its own dated amendment. Not yet sized against the real post-Planner codebase: `SLOT_IDS` gained a second brain-adjacent socket (`'planner'`) during that half, which changes the "does a second socket even exist yet" question `14-…` §5.2 was written against. Re-derive before building, the same discipline `23-…`/`24-…` held to for their own WPs.
...
None of the remaining three buildable WPs (WP30's If/Then half, WP32, WP34) is fixed as "the" next one by this file — read `18-…` §3/§7 and confirm with the user which to start.

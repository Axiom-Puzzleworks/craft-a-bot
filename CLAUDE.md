...
**Where we are (2026-08-21):** V1.0 ("My Very First Agent") shipped WP0–WP10. Day 2 Phases A–D are closed (WP11–WP25): engine trust fixed, the brick contract opened, teaching-aid UX and the professional Workshop both shipped, plus a full governance layer (policy cards, Safety Brick v2, three scenario cards). **Phase E (expansion era) is fully closed**: WP26, WP27, WP28 and WP29 (multi-agent core — `SessionGroup`, the Playroom's `forAgent`, group-altitude events, a minimal Workshop surface; full design of record and close-out in `docs/design-day2/23-MULTI-AGENT-DESIGN.md`) are all done. **Phase F ("Agent Builder" kit, WP30–34) is under way, with WP30, WP31 and WP32 now all fully done**: WP31 (Radio brick + the Robot Friends duo experience, including the ASI07 spoofed-message scenario), full design of record and close-out in `docs/design-day2/24-ROBOT-FRIENDS-DESIGN.md`; WP30 (Planner brick, its live checklist and eighth leaflet chapter; the If/Then brick, its `reflexes` socket, and its own ninth leaflet chapter), per its own dated amendments in `14-…` §5.1 and §5.2; and **WP32 (Librarian + Connector bricks) is done too** — the Librarian's own tenth leaflet chapter ("Ask before you guess," on a genuinely new goal card) and Connector's own confused-deputy mini-scenario (`starter/false-alarm`, shipped as a side quest rather than a chapter), per its own dated amendment in `14-…` §5.6. **WP33's identity badge half (the Passport, and its Agent Card export) is done too** — sized down before it started, per its own dated amendment in `14-…` §5.8. WP33's other half (kit-line packaging) is now unblocked — it needed WP30's and WP32's actual bricks to exist before there was a bundle to curate, and both are done — but not yet started. WP33's remainder and WP34 remain — see "Next up" below.
...
Next up per `18-…` §7 (most recent entry: item 27, 2026-08-21): Phase E is fully closed (WP26–29 all shipped). Phase F has WP30, WP31 and WP32 fully done, and WP33 half done:

- **WP33 (remaining):** kit-line packaging (`14-…` §5.8's own dated amendment) — box art per expansion pack, the 5–11 kit as a curated pack bundle. No longer blocked on missing content — WP30's and WP32's actual bricks now exist — though no purchase/licensing mechanism exists in this local-first, no-backend app for "purchasable" to mean anything yet regardless.
- **WP34:** Workshop maturity (audit centre) — see `18-…` §3 for its own row and DoD.

Neither of the remaining two buildable WPs (the rest of WP33, WP34) is fixed as "the" next one by this file — read `18-…` §3/§7 and confirm with the user which to start.

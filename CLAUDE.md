...
**Where we are:** V1.0 ("My Very First Agent") shipped WP0–WP10. Day 2 begins now: fix engine trust, open the brick contract, then the teaching-aid UX and the professional Workshop.
...
Next up per `18-…` §7: **WP11 + WP12 together** (behaviour fixes C1–C8 landing with their tests), then WP13 → WP14 (open brick contract, gated on golden-trace migration), then WP16 with WP19 in parallel.

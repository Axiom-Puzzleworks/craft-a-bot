...
**Where we are (2026-08-20):** V1.0 ("My Very First Agent") shipped WP0–WP10. Day 2 Phases A–D are closed (WP11–WP25): engine trust fixed, the brick contract opened, teaching-aid UX and the professional Workshop both shipped, plus a full governance layer (policy cards, Safety Brick v2, three scenario cards). **Phase E (expansion era) is fully closed**: WP26, WP27, WP28 and WP29 (multi-agent core — `SessionGroup`, the Playroom's `forAgent`, group-altitude events, a minimal Workshop surface; full design of record and close-out in `docs/design-day2/23-MULTI-AGENT-DESIGN.md`) are all done. **Phase F ("Agent Builder" kit, WP30–34) is under way: WP31 (Radio brick + the Robot Friends duo experience, including the ASI07 spoofed-message scenario) is done**, full design of record and close-out in `docs/design-day2/24-ROBOT-FRIENDS-DESIGN.md`, **WP33's identity badge half (the Passport, and its Agent Card export) is done too** — sized down before it started, per its own dated amendment in `14-…` §5.8 — **and WP30 is now fully done as well**: both the Planner half (the brick, its live checklist, and its own eighth leaflet chapter) and the If/Then half (the reflex mechanism, the brick — fitted to a new `reflexes` socket after "share `mobility` with Actions" was proven wrong mid-build — and its own ninth leaflet chapter), per their own dated amendments in `14-…` §5.1 and §5.2. WP33's other half (kit-line packaging) stays genuinely blocked, though on less now: it needed WP30's and WP32's actual bricks to exist before there was a bundle to curate, and WP30 is done, so only WP32 stands in the way. WP32, the rest of WP33, and WP34 remain — see "Next up" below.
...
Next up per `18-…` §7 (most recent entry: item 26, 2026-08-20): Phase E is fully closed (WP26–29 all shipped). Phase F has WP30 and WP31 fully done, and WP33 half done:

- **WP32:** Librarian + Connector bricks (`14-…` §5.5–5.6) + scope-permission leaflet chapter.
- **WP33 (remaining):** kit-line packaging (`14-…` §5.8's own dated amendment) — box art per expansion pack, the 5–11 kit as a curated pack bundle. Genuinely blocked, not merely unstarted: it needed WP30's and WP32's actual bricks to exist before there was a bundle to curate; WP30 is done now, so only WP32 stands in the way, and no purchase/licensing mechanism exists in this local-first, no-backend app for "purchasable" to mean anything yet regardless. Do not pick this up before WP32 has shipped something real.
- **WP34:** Workshop maturity (audit centre) — see `18-…` §3 for its own row and DoD.

Neither of the remaining two buildable WPs (WP32, WP34) is fixed as "the" next one by this file — read `18-…` §3/§7 and confirm with the user which to start.

...
> **Amended 2026-09-06 (WP68, `57-HARNESS-AT-SCALE.md` §4.2–4.4).** At scale: `--jobs <n>` runs cells in a pool of worker threads over the harness's own built worker, the report placed by cell order so it reads the same as `--jobs 1`; `--shard i/n` runs the i-th of n slices and marks the report a shard; `--seeds a-b` replaces the file's seeds; `--resume` reuses every cell a stopped run finished whose run still verifies (`<out>/cells.jsonl`) and runs the rest; `craftabot merge --file <campaign.json> <report.json>…` folds shard reports into one with the gates over the whole, refusing different campaigns, overlapping shards and a fold over the budget; `craftabot index --rebuild --out <dir>` rewrites the store's `index.jsonl`, which a listing reads instead of opening every run directory. `runCampaign` gained one seam for it all: `execute(cell, run)` with `concurrency`.
...
>
> **Amended 2026-09-02 (Stage C).** The renderers, the baseline file, `craftabot campaign` and the CI job landed as §4.5–§4.8 describe, with four notes. **Cells carry their scenario's `tags`** (`CampaignCell.tags`), so a gate's `where` selects over cells alone and a renderer can list a failed gate's runs and threat tags without the campaign in hand — `evaluateGate(gate, cells, baseline?)` lost its campaign parameter. **SARIF** is validated in the suite against the vendored official 2.1.0 schema (`packages/evals/schemas/sarif-schema-2.1.0.json`, from schemastore, draft-07) with `ajv` in `strict: false, validateFormats: false` mode (the schema declares `uri`/`date-time` formats and the repo ships no `ajv-formats`); **JUnit** is checked structurally, as §4.5 said it would be. **The baseline file is compared as a campaign, not as bytes**: prettier owns the committed JSON's layout, so `baseline-file.test.ts` parses both and deep-equals them; `npm run campaign:baseline` in `packages/evals` regenerates it. **The CI job** is a second job beside `build`, not a step in it, so the campaign's own artefacts and SARIF upload have their own `security-events: write` permission and the main job's permissions stay as they were; `github/codeql-action/upload-sarif` is the one third-party action added, GitHub's own. Gate: evals 8 files / 81 tests; harness 9 / 79 — `craftabot campaign` over the baseline at one seed writes the report, all three renderings and sixteen run directories, and the red edit exits 1 under `--strict` with the two failing gates on stderr.

---

> **Amended 2026-09-02 (Stage D).** Persistence and the screen landed as §4.9 describes, with three notes. **The store keeps an envelope, not the report's schema**: `StoredCampaignReport` in `core/schemas/records.ts` carries the fields a list shows (`campaignId`, `title`, `createdAt`, `passed`, `gatesPassed`/`gatesTotal`, `cells`) and the report itself as opaque JSON, because `core` cannot depend on `@craftabot/evals`, where `campaignReportSchema` lives; the workbench's `campaign-cells.ts` (`envelopeFor`/`reportFrom`) validates on the way back out with `parseCampaignReport`, and a row that no longer parses is simply not opened. The four methods (`putCampaignReport`/`getCampaignReport`/`listCampaignReports`/`deleteCampaignReport`, newest first by `createdAt`, outside `evictOldRuns`, cleared by `clear()`) are on all three stores — memory, IndexedDB at `DATABASE_VERSION` 4 with an upgrade test from a real v3 database, file at `campaigns/<id>.json` — and in `describeStorageContract` as seven cases. **The screen edits the campaign as JSON** (a textarea with `campaignSchema` diagnostics, the shipped baseline loaded by default, a file import) rather than the form §4.9 sketched — the campaign *is* the file CI runs, and a form that could express only some of it would have been a second dialect; a live brain disables the run with a note naming the harness command and `budget`. **No Worker was needed**: the shipped twenty-seed baseline (320 cells) runs on the main thread with the Eval Matrix's macrotask yield in a few seconds with the count painting, so the Worker `27-…`'s row allowed for stays unbuilt. The drill keeps traces in memory for the visit and persists only the run a person opens (through `putRun`/`appendEvents`/`persistRunSummary`, as the Eval Matrix does); a stored report reopened after reload shows gates and grid but says its cells cannot be opened. Gate: core 38 / 575; harness 9 / 86; workbench 66 / 838 plus four e2e journeys — the baseline at one seed runs green (13/13), a cell opens in the Run Lab named for its cell, the report is listed after a reload and reopens; the red edit fails `guard-holds:keep-the-secret` and `guard-holds:party-line` and leaves `guard-holds:warning-sign` green; a live brain is refused with the pointer; a non-campaign is refused with the reason. Full e2e 144/145, the one failure `incidents.spec.ts` under load (the known debt, `27-…` §8 item 1), green alone.

> **Amended 2026-09-02 (WP42):** the baseline gained two guards — the `local-llama-guard` and `azure-content-safety` stacks, each `starter/safety` + `workshop/guard` unplugged, `for` every scenario — so the committed file is 640 cells (32 at one seed) with the same thirteen gates; `RunCampaignOptions.packs` registers what a stack needs beside the starter pack (`30-SECOND-VENDORS.md` §6, §7 stage D).
...
>
> **Amended 2026-09-12 (WP97, `89-STACKS.md` §4).** `guards[].stack?: string` names a registered stack; `resolveCampaign` resolves it once into `components` (the stack's loop and egress fits after the guard's own) and `group` (the stack's chokepoint half unless the guard names its own), so a cell never sees the field and the report's `guard` id stays the guard's. A stack no pack ships is refused at prepare. WP94's `components?` and this ride together: a guard may fit the bricks a stack does not replace (a Monitor Judge) beside a stack.

> **Amended 2026-09-06 (WP71, `60-CONTROL-ROOM-V2.md` §4.1).** `/workshop/campaigns` on the Control Room: the verdict and every gate a `Lamp`, a `Meter` for every rate gate (the required rate parsed from the gate's own `>= 0.9`), the cohort comparison and the obligation table on `Matrix`; the e2e reads the lamps' words and `data-status`.

> **Amended 2026-09-07 (UX-5, `docs/manual/UX-AND-GAPS.md`).** `PackManifest.campaigns?: ShippedCampaign[]` — a pack names the campaigns it ships (`{ id, title, description?, campaign() }`), built from its own content when asked for; the three desk packs register their baselines (the Advice pack two, with the complaints desk's). The Workshop's Campaigns screen offers every shipped campaign in a picker beside *Load baseline*, opens on `?baseline=<id>` (each desk page links there as *Run this desk's campaign*) and, in the Playground section, on the Advice Desk's rather than the Playroom's injection baseline — which stays first in the list, shipped by the harness rather than a pack (`lib/workshop/shipped-campaigns.ts`). Content, not mechanism: `core` holds the campaign as an untyped object because the file's shape is `@craftabot/evals`'s, and the host parses it as it parses an import.

> **Amended 2026-09-10 (WP77, `64-TARGET-DESIGN-V5.md` §6.6.1, D7; `65-…` §3).** The Worker §4.9 and stage D declined is built, because a bank in motion cannot exist without it and the desks' baselines (930 cells) made the macrotask yield's case moot: the Campaigns screen no longer runs anything — `lib/state/campaign-runner.svelte.ts` queues a campaign onto one module Worker (`lib/worker/`), which hosts the same `runCampaign` with the same packs and plan chain and posts progress, traces and the report back; the store persists the report on the main thread as it lands, whether or not the page is open, so the tab answers throughout and a run survives a walk to another screen. **Cancel** and the estimate are unchanged. The report is byte-identical to a main-thread run (a test fixes the clock and the report id on both sides and compares); the Worker's chunk is counted apart by the budget (`01-…` §8's note). The risk row below ("320 cells freeze the Workshop tab") is closed by it.

> **Amended 2026-09-11, later still (WP82, `74-GATES-AND-REPORT-V3.md`).** The report is **v3**: `CAMPAIGN_REPORT_SCHEMA_VERSION = 3`, every earlier report keeping its version on read (WP61's rule) and reading with the new panes empty. A cell carries `decision` (the last `decide`'s outcome and the truth's verdict). A `parity` gate may name a `metric` from `@craftabot/metrics` with `stratify`, `confidence` and `power`; its verdict carries `interval`, `n`, `p`, `method`, `underpowered` and, when inconclusive, `reason`. A `drift` gate compares the selected cells with a reference — the baseline report, or a book campaign's population — by `psi`, `ks`, `outcome-mix`, `agreement` or `fairness`, under `atMost`. The summary gains `fairness[]` and `drift[]` off the verdicts; the scorecard two sections; the Campaigns screen a Fairness pane and a Drift table. JUnit and SARIF unchanged in shape.
>
> **Amended 2026-09-11, later (WP81, `70-CONTEXT-AND-ONTOLOGY.md` §6).** The context ladder is an axis: `contexts?: ContextSpec[]` multiplies every cell (a scenario's or a book's) by the rungs named, the rung reaching the world at `create` as `config.context` beside the knobs — an injected world, a plain one, a duo's and a book cell's journey alike; a cell carries `context` (the rung's id) only when the campaign named contexts, so a campaign without them is byte-identical to before; the summary's slices and case rows carry the rung, the scorecard's cells and cases tables show it beside the scenario, and `gateWhereSchema.context` selects on it — a `parity` gate across `context` is how "does the ontology help the bot decide alike?" is asked. `docs/schemas/` regenerated.
>
> **Amended 2026-09-11 (WP80, `64-…` §6.6.3; `73-LENDING-WORKFLOW-AND-BOOKS.md` §5).** A campaign may draw its cells from a **book through a workflow** instead of from scenarios: `source?: { kind: 'book'; workflowId; configuration?; book? | population?: { seed, size, periodDays? }; filter?; limit? }`, with `scenarios` empty when a source is present. The cells are items × builds × guards × brains, the first seed seeding the seats; `prepareCampaign` draws the book through the workflow's own `book` (or takes it inline), so a worker of the pool draws the same bytes. `builds[].overrides.configuration` names the workflow's configuration a build runs — an autonomy level applied to the journey — and the report's `builds[]` carries it beside the knobs. A cell gains `item` and `workflow` (the run id, the configuration and level, the stage statuses, the touches, the decisions with their level, the breaches); the summary gains `humanLoad[]` by build (touches per case with its interval, the unattended rate, the ceiling-breach rate with its interval), rendered as *Human load* in the scorecard and on the Campaigns screen. A book cell is scored over the workflow's events followed by every agent run's with the desk's truth as the journey left it; its `runId` is the last agent run's. `craftabot book run` writes such a campaign and runs it; `craftabot sweep` multiplies a file's builds by a knob's values; the Campaigns screen's Books and Sweeps panels do the same and queue the result onto the Worker. Schema versions unchanged; `docs/schemas/` regenerated.
>
> **Amended 2026-09-10 (WP78, `64-…` §6.6.2).** `builds[].overrides.knobs?: Record<string, number | string | boolean>` — the world's knobs, not a spec override: stripped before the spec is built and handed to the world at `create` as `config.knobs` (an injected world and a plain one alike, the duo's too), so a desk's policy is a build axis and a sweep over one threshold is one campaign with one build per level. The report's `builds[]` entry carries the `knobs` it ran with, so a slice by build reads as a slice by knob. Both schemas gain the optional field; versions unchanged, `docs/schemas/` regenerated.

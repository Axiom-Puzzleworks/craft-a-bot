...
| D23 | C | **The Spec Lab and the Boundary map crashed on two bricks of one kind** (`routes/workshop/spec/[agentId]/+page.svelte`, `components/control-room/Boundary.svelte`; found 2026-09-06, WP64 stage C; fixed there). Both listed a bot's bricks in an `{#each}` keyed on `slot + kind`, which WP40's stack made non-unique the moment "Fit another" put a second Monitor Judge in the safety socket — Svelte threw `each_key_duplicate`, the page went dead and every reactive readout after it froze, and the Spec Lab of any such bot crashed on load. Nobody had fitted two of a kind until the Compliance Watchbot preset did. Both keyed by position now; held by `live-counterpart.spec.ts`. | `apps/workbench/src/routes/workshop/spec/[agentId]/+page.svelte`, `apps/workbench/src/lib/components/control-room/Boundary.svelte` |
| D22 | C | **The engine's resume tests raced the clock on CI** (`session/agent-session.test.ts`, the `resume` block; found 2026-09-06 on `main`'s CI after WP66 merged, fixed in WP65's PR). `playable(40)` scripted a run that wins after forty ticks at a 1 ms tick delay under the default thirty-tick budget; on a fast Linux runner those ticks were over inside a couple of `vi.waitFor` polls (50 ms apart), so by the second pause the run had *finished* and `status` read `finished`, never `paused`. Windows ran slower and passed. The runs now script two thousand ticks with a budget to match, more than any poll cadence gets through. Every `main` push run since 2026-09-05 evening had failed on it while the PR checks for the same commits passed — a reminder that a check on a PR and a check on `main` are two runs. | `packages/core/src/session/agent-session.test.ts` |
| D21 | B | **A trace file's digest verified only by coincidence** (`persistence/trace-export.ts`; found 2026-09-06, WP65 stage A; fixed there). `buildTraceFile` took the digest over the events as emitted, then parsed the file through `traceFileSchema`, which rebuilds each event with its keys in *schema* order — so `verifyTraceDigest`, hashing the parsed events, agreed only while the engine emitted keys in schema order. The two golden traces and every Kit run happened to; a fork (`forkedFrom` emitted before `egress`, declared after it) did not, and neither did `run.started.principal` until its schema position matched. Nobody noticed because the Workshop's badge verifies over the store's events, never a parsed file, and no test exported a fork. Fixed by parsing first and digesting the events the file carries (the bundle's group events the same way); a trace whose order already matched digests exactly as before. Held by `session/principal.test.ts` (a flipped `by` fails). | `packages/core/src/persistence/trace-export.ts`, `bundle.ts` |
| D20 | C | **The starter Sense and Actions bricks default to the Playroom's ids** (`brick-kinds.ts`; found 2026-09-05, WP53 stage A; **fixed 2026-09-05 at the Phase M exit review**). On any other world — the Workshop room since WP28, the Front Desk — a bot did nothing until the player ticked that world's own channels and actions in the brick panels; no e2e had ever played the Workshop room's cards through the Kit, which is how it went unnoticed, and it bit the harness's desk kit in WP55 (worked around by qualifying ids). The bench question was answered with the smallest honest rule: `benchStore.setGoalCard` re-points the two bricks when the card changes *world* — whatever the bot had fitted that the new world also offers (by bare id), or everything the new world offers when nothing carries over — and leaves them alone when the world is the same. Held by `bench.svelte.test.ts` (Playroom → Front Desk → Playroom, and a same-world switch) and by `desk.spec.ts` (the desk's channels already ticked when the panel opens). The kinds' defaults still name the Playroom, which is right for a new bot on the Kit's first card. | `apps/workbench/src/lib/state/bench.svelte.ts` |

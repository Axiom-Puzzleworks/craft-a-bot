...
| D20 | C | **The starter Sense and Actions bricks default to the Playroom's ids** (`brick-kinds.ts:647,670`; found 2026-09-05, WP53 stage A; open). On any other world — the Workshop room since WP28, the Front Desk now — a bot does nothing until the player ticks that world's own channels and actions in the brick panels; no e2e had ever played the Workshop room's cards through the Kit, which is how it went unnoticed. A bench question (re-default those bricks when the card's world changes?) for the Phase M exit review; the Playground's cards meet it at WP60. | `packages/packs/starter/src/brick-kinds.ts`, `routes/bench/[agentId]` |

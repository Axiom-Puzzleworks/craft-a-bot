...
> **Amended 2026-09-12 (WP96, `85-COMPONENTS.md` §4).** No new event. **`action.performed.redacted?`** — `{ guardrailId, finding? }` when a `redact` verdict on the pre-act chain rewrote the call's `text` before it ran; `arguments` is what was said, the bot's own words are on the `decision` event. Absent on every action written before, and on every action no guard rewrote. The `redact` and `annotate` allow-branch fields (WP94) are now applied: the session rewrites `text`, the shell's `note` disposition writes `verdictKind: 'annotate'` with its finding.
>
> **Amended 2026-09-12 (WP95, `69-WORKFLOWS.md` §10).** No new event. **`stage.completed.guards.verdicts?`** — the stage's boundary chain's verdicts in order (`boundaryVerdictSchema`: `{ guardrailId, point, verdict, componentId?, policyCardId?, reason?, approved? }`), absent when the stage had no boundary guards. A boundary check is written as `guardrail.checked` / `guardrail.tripped` with `point: { kind: 'stage-in' | 'stage-out', at: stageId }`, on the workflow's events at `stage-in` and where the stage's `stage.completed` goes at `stage-out`. `GuardrailContext.stage?` (`{ id, point, input, output? }`) is the one contract addition, present only on a boundary check.
>
> **Amended 2026-09-12 (WP94, `85-COMPONENTS.md` §4).** No new event. Two optional fields on **`guardrail.checked`** and **`guardrail.tripped`** — **`componentId?`** and **`point?: { kind, at? }`** — written only when a `GuardrailComponent` compiled the guardrail (stamped on the `Guardrail` beside `policyCardId` and copied through by the session, never derived from the id); every trace written before is unchanged. And the *allow* branch of `GuardrailVerdict` gains **`verdictKind?: 'redact' | 'annotate'`**, **`finding?: { category, label?, confidence? }`** and **`redactedText?`** — an allow that noted something, for WP96 to act on; the chain runner treats it as an allow.
>

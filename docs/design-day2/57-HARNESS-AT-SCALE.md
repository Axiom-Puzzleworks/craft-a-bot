...
> **Stage B landed 2026-09-06.** Resume and the index (§4.3–4.4). `execute` takes the runner's own way as its second argument (`execute(cell, run)`), so a host can reuse a cell or delegate it without owning a second cell runner. The harness: `<out>/cells.jsonl` — `{ ordinal, runId, digest, cell }` per finished cell, the digest over the events *as the store reads them back* (parsed, keys in schema order — `12-…` D21's lesson applied before it bit) — appended in the same ordered chain that writes the run; `--resume` (`reusableCells`: a line whose run directory exists and whose `events.jsonl` still digests to the line) reusing those cells through `execute` and running the rest, `onCellDone` saying which, refused without the runs kept; a write that fails now fails the campaign instead of vanishing into the chain. The store: `index.jsonl` — `{ id, agentId, agentName, goalCardId, outcome, summary, digest? }`, one line per `putRun`, one more with the digest per `putRunSummary`, a `deleted` line per `deleteRun` (eviction included), the last line for an id winning; folded once per store instance and kept in step; `listRuns` and `listRunSummaries` read it and open only what they return; a store with no index rebuilds one on first read; `readIndex`, `rebuildIndex` and `craftabot index --rebuild --out <dir>`; a summary with no run record beside it (the storage contract allows one) still lists; `clear()` drops the index. The proofs (§11 items 3–4, `campaign-resume.test.ts`): a run stopped after ten cells resumes with nine reused, one — tampered — run again, every cell present once and the report equal to a whole run's; the maintained index equal to one rebuilt from disk, a fresh store folding the same, the summaries listing from it, a deleted run leaving it, a missing index rebuilt on first read; the CLI's rebuild. The storage contract suite holds over the indexed store. Gate: root lint, every workspace's tests, the build, the evals baseline, the default e2e and the visual set.

> **Stage C landed 2026-09-06 — WP68 closed.** Live trailing, the harness half (§4.5): `lib/workshop/sink-import.ts` — `parseSinkLines` reads the file sink's JSONL back into a record and its events (a `{ kind: 'run', record }` line honoured, a provisional record folded from the events otherwise, `IN_PROGRESS` until a `run.finished` line, a group's export refused, a line it cannot read counted), `looksLikeSinkLines` telling it from a trace file; the Run Browser's import takes `.jsonl` beside the trace file, replaces the run's events with the file's (the file is the whole of what the sink has written) and says *still going … re-import to catch up* or *caught up*; the Run Lab's header says *still going — re-import the sink file to catch up* on an imported run with no end. The scale check (§4.6): CI's campaign job runs the injection baseline with `--seeds 1-63 --jobs 4 --no-keep-runs` — 2,016 scripted cells — as a strict step; on the development machine (20 cores) the same run takes **2 s** with four workers and 2 s on one lane, the cells being small; the CI runner's figure is the step's own duration on the workflow page, well under the ten seconds this note stakes. The proofs (§11 items 5–6): the golden trace's events minus its `run.finished` line imported as a run still going, the Run Lab saying so, the finished file re-imported and caught up (`sink-import.spec.ts`); the parser over the same golden (`sink-import.test.ts`). Close-out: `42-…` §3 and §8 items 50–52, `41-…` §12, `28-…` §4.7, `17-…` §4.3, `37-…` §7, `CLAUDE.md`, `README.md`, the harness README. Gate: root lint, every workspace's tests, the build, the evals baseline, the default e2e and the visual set.

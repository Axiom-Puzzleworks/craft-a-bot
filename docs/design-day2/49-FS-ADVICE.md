...
> **Amended 2026-09-11 (WP85, `76-FRAUD-AND-ADVICE-WORKFLOWS.md` §4).** The desk gains what the advice journey needed, all content: the `work-item` layout (`adviceCaseFromItem` — the request's own customer with the balance they hold, the answers from the topic's profile, the suitable set recomputed in truth; `assembleAdviceCase` now shared with the sixteen hand-built layouts), `run-fact-find` (every required question not yet asked, at once — the structured form), `check-suitability` (the suitability rule over the topics *asked so far*, the result on the customer record as facts), the answers held on `extra.advice.answers` (never in a sense; a topic not asked reads as its cautious default), `recommend-product` recording the product on the file, and two predicates — `advised`, `warnings-given`. The manifest lists the workflow (`fs-advice/advice`), and the baseline's builds list the two new actions, so `campaigns/fs-advice-baseline.json` is regenerated; every card, scenario, evaluator and gate is unchanged.

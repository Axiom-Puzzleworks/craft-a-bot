...
> **Amended 2026-09-05 (WP61, `50-DOMAIN-METRICS.md` §4.7):** `telemetrySeries` takes a third argument, `TelemetryExtras { evaluations?, reports? }`, and a bucket carries `series`/`seriesSamples` — pass rates, label shares, case-metric means and cohort spreads by day; `driftIn` flags a `series` step change with `seriesThreshold` (0.3). The Telemetry page lists the domain series. Defaults and the two older flags unchanged.
...
> **Amended 2026-09-06 (WP68, `57-HARNESS-AT-SCALE.md` §4.5).** The harness half left recorded above is built as an import, not a stream into the Run Lab: the file sink's JSONL of a run in progress imports into the Run Browser and the Run Lab says the run is still going; re-importing catches up. G35's "harness-side live trailing" is retired on those terms.

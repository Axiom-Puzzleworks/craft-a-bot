...
**The sweep** (`desk/src/synthetic-sweep.test.ts`) walks `packages/**` and `campaigns/**` for `*.json`, `*.jsonl`, `*.md` and `*.ts` files under any `fixtures/`, `baselines/`, `cassettes/`, `scenarios/` or `corpus/` directory, plus every `campaigns/*.json`, skipping `node_modules`, `dist`, `coverage` and `__snapshots__`, and asserts no issue. A second test plants a Luhn-valid Visa number in a temporary file and asserts `checkSynthetic` refuses it, so a sweep that stops biting fails the build. `describeConformance` does not run `checkSynthetic` (a pack's fixtures are files, not manifest content) — the sweep is repo-wide, once.
...
> **Stage B landed 2026-09-05.** `DeskTruth` and `DeskCase.truth?` in `@craftabot/desk`; `buildState` returns the state and the truth as two values and the instance keeps `truth` in its closure beside `state`, so `snapshot()` (and so `world.changed`) cannot carry it; `truth()` is on the instance only when the case produced one, so `'truth' in instance` is false for the golden desk and its trace is byte-identical. `truthfulTestDeskSpec`/`truthfulTestDesk` beside `testDesk` (the visitor's real standing drawn from the seed; `facts.outcome` admit or refuse) with tests for the clone, the snapshot, every sense, the progress line, the reset and the spread of outcomes across seeds. The Front Desk's truth: the `visitor-truth` record and `facts.right_decision`, held by a unit test that looks the visitor up and reads every sense. `checkDesk`'s `desk.truth-never-sensed` and `desk.truth-not-in-snapshot` (§4.3) with the kit's own eleven-line mulberry32 and `truthOnlyValues` over the truth's leaves less the records' leaves, proven on a keeping desk, a leaking desk (the message names the sense and the value) and a snapshotting desk; the Front Desk passes it under `describeConformance` over a hundred seeds and its script. `WorldStage`/`DeskView` take `truth?: unknown`, `DeskView` narrows it by shape and `CaseFile` gained `facts` inside the same flap (`desk-truth-facts`, `desk-truth-fact-{key}`); the Run Lab and Compare pass `shown.truth`; `kit-never-truth.test.ts` reads the three Kit routes' source and the two Workshop routes' and holds §3's seventh principle. `desk.spec.ts` sees the flap at the last turn, sees it go when the scrubber moves to 0, and sees none in the Kit's replay.

> **Stage C landed 2026-09-05 — WP54 closed.** `packages/desk/src/synthetic.ts` (§4.5): nine primitives, each over the caller's `random` alone, with `luhnCheckDigit` exported so a test can prove the card number is off by one; a thousand whole people pass `checkSynthetic` in JSON and JSONL. `checkSynthetic` in the testkit (§4.6) with `luhnValid`, `ibanValid` and `reservedDomain` exported, each shape proven in `checks/synthetic.test.ts` from values built at run time (no card-shaped literal in the source) — including the negatives that keep the estate clean: a millisecond timestamp, a Luhn-valid number with no issuer prefix, a 26-digit run, an unkeyed date, `GB00`. `synthetic-sweep.test.ts` walks `packages/` and `campaigns/` for JSON, JSONL, Markdown and TS under `fixtures/`, `baselines/`, `cassettes/`, `scenarios/` and `corpus/` plus `campaigns/*.json` (skipping `node_modules`, `dist`, `coverage`, `__snapshots__`, `.svelte-kit`, `build`), found the whole estate clean on its first run — no tuning was needed beyond the issuer-prefix rule the note had already chosen — and plants a Luhn-valid Visa-shaped number to prove it still bites and masks what it found. One correction to stage B's property found by the Front Desk under `describeConformance`: a truth value that is also a queue item's id or title, or the desk's own nameplate, is not a secret (`sign-in` sits in every snapshot as the queue item's id), so `truthOnlyValues` subtracts the queue's and the nameplate's leaves as well as the records'. Hard rule 9 in `CLAUDE.md`; `10-…` §8 item 7 no longer a placeholder; `13-…` §7's note; `43-…` §4.8's note that the truth property is now the gate; the renumbering (`46-FS-BANK`, `47-FS-ADVICE`) in `42-…` §3 and §8; `41-…` §12's rows for §8. **WP54's DoD, checked:** the core session test (exactly once, no other event); the tenet-13 property over 100 seeds on the truthful test desk and the Front Desk; a planted Luhn-valid PAN fails the sweep and every existing fixture passes it; all three golden traces byte-identical. G24 and G34 retired.

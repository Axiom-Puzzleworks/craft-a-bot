...
**The alternative, recorded not chosen.** A Cloud Run proxy holding a service account would let a plain secret stand in for the token and hide Google entirely — and would make the guard a hidden dependency of a local-first toy with the cost on the maintainer instead of the builder. This was conditional on the regional hosts refusing browser origins (CORS was undocumented for `*.rep.googleapis.com`); §8's 2026-09-01 live-checkpoint note confirms they do not refuse it, so `credential.kind: 'proxy-url'` stays exactly what this paragraph always called it — a recorded alternative, not built.
...
>
> **Amended 2026-09-01 (Stage D, four real findings against the plan).**
>
> 1. **`controlHints` cannot cover `filters` at all — a real gap, not a stylistic one.** `schema-fields.ts`'s `describeFields` reads only `configSchema`'s own top-level shape (`Object.entries(internals(schema).shape)`, no recursion), and `controlFor`'s switch has no `'object'` case — a nested `z.object` field (`filters`, §4.3's own per-filter overrides) falls straight to `default: text`, and a hint keyed `'filters.injection'` is never looked up at all, since lookups are by literal top-level field name. This was never going to surface in a table read — it took building the kind and reading the panel's own source to find. Ten of the fourteen config fields (project/region/template, the three screen dials, confidence, `onFailure`, `timeoutMs`, `maxTicks`, `offline`) are flat and render correctly through `SchemaPanel`; `filters` does not, and currently shows as a plain, unhelpful text box. `starter/safety` already sets the precedent for this exact situation — a kind whose panel a hint set cannot fully cover reaching for a hand-written override (`14-…` §2.1's documented limit, `SafetyPanel.svelte`) — and the per-filter overrides want the same treatment. Not built this stage: a hand-written `ArmourPanel.svelte` is real UI work with no DoD test riding on it (the four overrides are fully reachable and fully tested at the config/engine level — `brick-kind.test.ts`, `scenario.test.ts` — only the bench's own visual control is missing), so it is left as a named follow-up rather than built under time pressure or silently shipped broken.
> 2. **`expansion-packs.ts` does not get a Cloud Armour row.** Read in full before touching it: its own header comment scopes it explicitly to "`18-…` §4's own merchandising table, verbatim" — the ages 5–11 kit line's Shelf section, Kit-facing by construction. The Armour Brick is Workshop-only and `audience: 'workshop'`-gated everywhere else specifically so the Kit never sees it (`15-…` §4's superset rule, `14-…` §2.1's own Stage C amendment); adding it to this file would put a Workshop accessory on the one screen guaranteed to render for every Kit user regardless of the Workshop door. `PartsTray.svelte`'s own generic `audience` filter (Stage C) already surfaces any Workshop-only kind wherever a builder can fit bricks, which is the correct, already-built place for this — no second listing was needed, and this row of §11's own "Builds" column is the one place the plan asked for something the codebase's own scope boundaries say should not exist.
> 3. **"Run `warning-sign` on the Demo Brain" is satisfied by a real scripted engine test, not a `demo-brain.ts` entry.** `demo-brain.ts`'s `SCRIPTS` map is not a general "any goal card, any script" registry — every entry is keyed to a specific *missing-capability* gap in the six/nine-chapter Kit teaching arc (`BotCapabilities`-driven: no actions, no sight, no memory…), and the Armour Brick has no such capability-gap concept (nothing in `BotCapabilities` tracks which safety-socket kind is fitted) and is never shown to the Kit that arc teaches. Extending it for a Workshop-only brick would be the wrong mechanism reaching into the wrong file. `scenario.test.ts` reuses the exact same `HIJACK_SCRIPT` `session/governance-scenarios.test.ts` and `armour-studio.ts` (stage B) both already use, driven through a real `createSession` call with `geap/armor` genuinely fitted (not guardrails passed loose), against a mocked Model Armor response — the DoD's own two moments, both proven: a real match stops the run legibly (`STOPPED_BY_GUARDRAIL`, a reason containing "sneaky instruction"), and an ejected credential (no token at all) fails closed legibly (`bad-token`, never a silent allow, never an `error` event).
> 4. **The leaflet-coverage test needed no exemption entry, because `armorConfigSchema` was never added to `BRICK_SCHEMAS`.** §4.8 anticipated "a written exemption for the kind, in the mould of `memory.strategy`'s" — but `coverage.test.ts`'s `BRICK_SCHEMAS` is a hand-maintained allowlist of six starter sockets, and `monitor/watchbot` (the other Workshop-adjacent, non-starter safety kind) was never added to it either; nothing in the coverage mechanism scans the registry itself. Adding `armorConfigSchema` there would have *manufactured* a need for the exemption §4.8 predicted rather than discovering one — the honest reading of the precedent Monitor already set is that a non-starter kind with no hand-written Kit panel simply stays outside this particular map, and `18-…`/`workbench` test suites confirmed nothing broke leaving it out.
>
> **Verified live**, real browser, both directions of the one thing that must never fail either way: with the Workshop door shut the tray lists twelve kinds and no Armour Brick; with it open, a thirteenth entry appears with its exact toy-face copy. `buildRuntimes`/`createSession`/`validateSpec` wiring (stage C) and the fitted kind itself both confirmed end-to-end through real code paths — `installedPacks.ts` registers `geapPack` the same way as every other pack, and `pack-geap`'s own suite (162 tests: shape, `describeArmorFitted`, `validateConfig`'s four codes — now registered in core's own closed `BuildProblemCode` enum alongside the reference designs' own per-brick codes — guardrail composition, the fitted-for-real scenario, and the offline golden trace) plus the whole workspace's (41 tasks) stay green throughout. One thing not completed: actually dragging the brick onto the chest socket through this session's own browser-automation tooling, which could not get a synthetic keyboard `Enter` to reach `PartsTray.svelte`'s own `onKeyDown` handler (focus landed correctly; the keydown never fired) — a tooling limitation on a pre-existing, unmodified drag-and-drop widget identical for every brick kind, not a Stage D defect; the same fitting path is what `scenario.test.ts` exercises directly at the engine level instead.
>
> **Amended 2026-09-01 (Stage E, four findings, one deliberately unfinished piece).**
>
> 1. **A real bug, found by trying to run the smoke script this stage builds.** Stage A's `client.ts` imported `../fixtures/clean.json` with no import attribute — fine under Vite/Vitest's own transform (every test suite that touched it, Stages A–D, stayed green), a hard crash (`ERR_IMPORT_ATTRIBUTE_MISSING`) under Node's own native ESM loader, which is exactly what `npm run smoke` uses to run the built `dist` directly. Fixed with `with { type: 'json' }`, the same fix Node 22+ (this repo's own floor) expects; every test suite re-verified green after. A reminder that Stage A's own tests, however thorough, only ever proved the code under one runtime.
> 2. **`credential.validate` stays undefined — its own fixed signature cannot do what §4.6 asked of it.** `(secret, fetch) => Promise<KeyCheck>` (Stage C, already shipped) carries no way to receive a brick's own `projectId`/`location`/`templateId`, and "fires a known-injection string at `sanitizeUserPrompt`" needs exactly those three, which exist only on a *fitted brick's own config* — a credential is shared machinery, a brick's config is not. "Test the guard" is built as a Settings-level feature instead (`geap-credential.svelte.ts`'s own `testTheGuard`, a small project/region/template form in `GeapCredentialCompartment.svelte`), calling `pack-geap`'s own `createModelArmorClient` directly rather than forcing a square peg through a round, already-committed seam. `BrickKindDefinition.credential.validate` remains available for a future credential whose check genuinely needs nothing beyond the secret itself.
> 3. **The OAuth Client ID does not exist, and nothing built this stage can make it exist.** §4.6 calls this "a one-time maintainer setup in a Google project" — a real action in the Google Cloud console only the project's own maintainer can take, the same category of thing this whole WP has deferred since stage B by the project owner's own explicit choice. `initTokenClient`'s own real shape is built in full (`geap-credential.svelte.ts`), sourced from `VITE_GEAP_OAUTH_CLIENT_ID` at build time, and every path around it is unit-tested with an injected `loadGis`/mocked `window.google.accounts.oauth2` (12 tests: empty state, a stored token surviving reload, no-client-id short-circuit, success storing the token with a real TTL, denial, a failed script load, ejection) — but the actual GIS popup flow, the one piece that needs a live client id and a live Google account, has not run once. Verified live in the browser instead, both directions: Workshop shut hides the whole compartment; Workshop open shows it, correctly reporting no client id configured and offering "Sign in with Google" with no crash.
> 4. **The key-leak sweep needed one line, not a new mechanism.** `key-leak.test.ts`'s own sweep already reads every value `vault.secrets()` returns, generically — planting `vault.set('geap', FAKE_GEAP_TOKEN)` alongside the existing `openai` plant was the whole change; the existing assertions covered it as soon as it existed, plus one new test making the geap-specific coverage explicit rather than incidental.
>
> **What is still not done, honestly.** Stage E's own third DoD clause — "one real `europe-west2` call succeeds from the built app" — is the same live-Model-Armor checkpoint stage B's own DoD named and the project owner chose to defer twice already. Nothing in stages C, D or E needed it to be safe to build, and nothing here would need to change once it lands: `smoke:geap` is written and gracefully skips without env vars (confirmed); `docs/geap-setup.md` §2 gives the fastest path to a real token (`gcloud auth print-access-token`) that does not require the GIS flow at all, precisely so the live check can happen without waiting on an OAuth client id too. Closing this stage for real — and `18-…` §7's own close-out entry for the whole WP — needs someone to actually run `smoke:geap` against a real project, or sign in through a real client id and press "Test the guard", and see what comes back.
>
> **Amended 2026-09-01 (the live checkpoint, closed).** Both deferred DoD clauses — stage B's "a real Model Armor verdict … CORS/latency numbers … go/no-go on direct browser calls," and stage E's "one real `europe-west2` call succeeds from the built app" — are now closed against the project owner's own GCP project (`gen-lang-client-0659895460`, `europe-west2`, template `cab-armour`), in two steps.
>
> First, `npm run smoke:geap` against a token from `gcloud auth print-access-token` (`docs/geap-setup.md` §2): a real `sanitizeUserPrompt` call returned `outcome: ok`, and the known-injection phrase tripped `pi_and_jailbreak` at `confidence: HIGH` — Stage A's wire-shape reconstruction (the dated note above, never sourced from a live capture) parses a real response correctly. This runs under Node, so it proves auth and wire shape but says nothing about CORS.
>
> Second, the actual go/no-go: the same token pasted into `/workshop/armour` (Stage B's own panel) with `Unplugged` switched off, running the sign-hijack test bench against the real endpoint from an actual browser tab — the one context where CORS is enforced at all. Eight `guardrail.external` calls succeeded (`decision`-hook `sanitizeModelResponse`, `outcome: ok`, latencies 262/126/145/79/81/92/80/84 ms — one slow first call, then steady), the Inspector's raw JSON showing `"endpoint": "https://modelarmor.europe-west2.rep.googleapis.com/v1/projects/gen-lang-client-0659895460/locations/europe-west2/templates/cab-armour:sanitizeModelResponse"` and no token anywhere in the record. **The go/no-go is direct: the regional host answers a browser origin with no CORS error, so `credential.kind: 'proxy-url'` (§4.6's "alternative, recorded not chosen") is decided *not needed* and stays unbuilt.** §10's CORS risk row is resolved to "confirmed clear" below.
>
> One operational note worth keeping, not a design finding: `gcloud model-armor templates create`/`describe` returned `PERMISSION_DENIED` for the project owner even with `roles/owner` bound and billing confirmed enabled; template creation succeeded immediately through the Cloud Console UI instead, and the CLI's `describe` kept failing identically even after a fresh `gcloud auth login`. Since the app's own REST client (same token, same project, same template) worked on the first live call, this reads as a `gcloud model-armor` CLI-surface quirk (a young command group), not a real IAM or billing gap — `docs/geap-setup.md` §1 keeps the `gcloud` commands as the documented path since they are the standard tooling, but the Cloud Console is the fallback if `templates create`/`describe` throws `PERMISSION_DENIED` despite correct roles and billing.
>
> **Amended 2026-09-01 (three post-close-out defects, found by the project owner actually using the credential flow rather than a smoke test).** The live checkpoint above proved the wire and the network path; it did not exercise the two credential UIs (Settings' battery, Armour Studio's own panel) as a real builder would, and three real gaps surfaced once someone did.
>
> 1. **`SchemaPanel`'s generic `'enum'` control read a `def.values` that real Zod 4 `z.enum(...)` schemas never have** (the values live under `def.entries`, an object map) — every fitted brick's own `screenObservation`/`screenDecision`/`screenResult`/`injectionMinConfidence`/`onFailure` dropdown rendered with zero options, unselectable, the moment the Armour Brick was actually dragged onto the chest socket. Every existing `schema-fields.test.ts` case reached the `'choice'` control through `z.union([z.literal(...)])`, which does carry `.values`, so this pre-existing bug in the generic bench panel (not specific to this pack) had simply never been exercised by a `z.enum` field before. Fixed in `schema-fields.ts` (reads `Object.values(def.entries)`), with two new tests against the real `zod` library rather than a hand-shaped mock — the mock is exactly what let the original bug hide.
> 2. **`VITE_GEAP_OAUTH_CLIENT_ID` in the repo-root `.env` was never read by the built app.** Vite's default `envDir` is wherever `vite.config.ts` lives (`apps/workbench/`), not the monorepo root — `docs/geap-setup.md` §3's own instruction to set it "at the repo root," matching `.env.example`'s existing one-`.env` convention, was simply wrong for how this app resolves env vars, and no rebuild could have fixed it. `apps/workbench/vite.config.ts` now sets `envDir` to the repo root explicitly. Caught before it shipped: this also made `vitest` read the same real `.env`, silently making two `geap-credential.svelte.test.ts` cases depend on whatever the developer's own machine has configured — fixed by stubbing `VITE_GEAP_OAUTH_CLIENT_ID` explicitly in those two tests rather than relying on its ambient absence.
...
|---|---|---|
| Regional `*.rep.googleapis.com` hosts do not answer browser origins (CORS) | **Resolved 2026-09-01 — confirmed clear.** Eight real browser calls to `modelarmor.europe-west2.rep.googleapis.com` from `/workshop/armour` all succeeded with no CORS error (§8's live-checkpoint note). | `credential.kind: 'proxy-url'` (§4.6) stays a recorded alternative, not built |

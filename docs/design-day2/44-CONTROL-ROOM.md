...
- **Sequential ramp** — five steps of one hue (blue, from the Eval Matrix's `rampStep`, now here) from `--cab-graph` to `--cab-blue`, each step naming its fill and whether its label is ink or cream; `sequential(value: 0..1)` returns the step. A second, green ramp for rates where up is good, keyed by a `direction`.
- **Diverging ramp** — red through cream to blue for "relative to a baseline" (`diverging(delta: -1..1)`).

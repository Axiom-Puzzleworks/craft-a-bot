...
> **Amended 2026-09-07 (WP73, `62-THE-TAIL.md` §2 item 3):** WP73's icons are the *instrument* set (a roundel per instrument, drawn before a `Strip`'s label through `Roundel.svelte`), not the lanes'. The lanes keep these glyphs; the trace/event set is a later wave (`63-…` §6). The two finishes arrive through `--cab-finish-graph`/`--cab-finish-metal` set on the Workshop's root, every instrument's rule the fallback (`62-…` §4.2).
...
|---|---|---|
| §6.15: `outside` includes `service-line`, `pdp`, `evidence-store` | Reserved kinds in the type; no entries produced until WP58, and WP70 (the OPA pack is a `guard-service` today). **Amended 2026-09-05:** `service-line` entries are produced since WP60 (a Connector's line); `pdp` and `evidence-store` stay reserved | Nothing registers them yet; the shape is fixed now so the map does not change when they arrive |
...
> **Amended 2026-09-06 (WP65, `55-PRINCIPAL.md` §4.3).** `BoundaryMap.human.principal` — typed `undefined` since WP57 as the slot WP65 would fill — is now the `Principal` on the run's `run.started` when the fold is given events and the host named one; the figure on the ring has a name. The Run Lab's Chain (`55-…` §4.5) is the component `44-…` §4.3's set named.

> **Amended 2026-09-06 (WP71, `60-CONTROL-ROOM-V2.md`).** §7's deferrals are built: every Workshop screen is on the system (§4.1 of `60-…`), the dashboard's sparkline is a compact `Tape`, and the visual-regression pass is diffed in CI on Linux baselines with the Windows ones beside them (§8's two rows). Axe now covers every Workshop route.

> **Amended 2026-09-11 (WP86, `77-PIPELINE-AND-BOUNDARY.md` §5).** The Boundary is rewritten over a layout engine, `lib/control-room/boundary-layout.ts`: the outside nodes evenly around the circle in kind order from the top on a radius sized to the widest box's chord (§4.5's fan of 22° per kind wrapped the circle once the bank had ten lines); every label collision-tested against every box placed before it and pushed outward along its own angle with a leader line when it would overlap (UX-7's fix); a node's box sized by its wrapped lines. The map gains a second ring per workflow (`BoundaryMap.workflows`, `workflowRing`), each stage drawn as its actor — the bot's mark, a cog for a rule, the person, a line's socket — and lit by the run when the map is over a Pipeline. Principles 4 and 5 hold: the component draws what the layout placed, and the layout is a pure function a test asserts the invariant over.

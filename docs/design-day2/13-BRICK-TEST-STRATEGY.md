...
> **Amended 2026-09-05 (WP53 stage C, `43-DESK-WORLDS.md` §4.8):** the kit gained `checkDesk(world, fixture?)` for every world with `view: 'desk'` — a `DeskWorldState` by core's own guard for every layout, a `riskTier` on every action, purity over create/observe/perform with `Date`, `Math.random`, `crypto` and `performance` replaced by throwing stubs for the duration (the egress guard's throwing-`fetch` idea, applied to the clock and the dice), `inject` present and honest about the four kinds against `acceptedInjections`, no `special-category` record in the opening snapshot or a sense's text on a desk that declares no `purpose` (the first cut; WP54's truth property is the real gate), and `reset()` returning what a fresh `create()` gives — with `checkWorld` run first when the fixture carries scripts. `describeConformance` runs it for every desk a manifest ships, from `PackConformanceFixture.desks` keyed by world id (a desk with no entry is checked with the defaults). Proven against three hand-written failing desks in `checks/desk.test.ts` — the kit depends on `core` only and does not use `@craftabot/desk` to build its fixtures, so it tests the contract, not the runtime.

> **Amended 2026-09-05 (WP54 stage A, `45-TRUTH-SYNTHETIC.md` §4.1):** `checkEvaluator` gained two checks. `evaluator.reads-truth` — an evaluator declaring `reads: ['truth']` is handed a fixture input carrying truth and must return a different result from the same input with truth removed (proof it reads it; the fixture must carry at least one such input, and the check says so if none does). `evaluator.truth-hidden` — an evaluator that does not declare it is handed an input whose truth is a planted sentinel, both stripped (as every run path strips it) and unstripped (as a forgetful caller might), and the sentinel must never surface in a result. The kit is `core`-only, so it mirrors governance's `inputReadableBy` in eleven lines rather than importing it. Proven in `pack-testkit.test.ts` against a peeking evaluator, a blindfolded one and the example pack's honest truth reader.

> **Amended 2026-09-05 (WP54 stages B–C, `45-TRUTH-SYNTHETIC.md` §4.3, §4.6):** `checkDesk` gained the tenet-13 property — `desk.truth-never-sensed` and `desk.truth-not-in-snapshot` over a hundred seeds (the kit's own mulberry32, so it still depends on `core` alone), the truth-only values derived from the truth's leaves less the records' leaves, checked against every sense, every progress line and the snapshot at the opening and after every fixture script; a desk with no `truth()` passes trivially. The `purpose-classification` first cut stays as the floor beneath it. And the kit gained `checkSynthetic(files)` — the shapes real identifiers have, refused with a masked report — with `synthetic-sweep.test.ts` running it over every fixture, cassette, scenario, baseline, corpus and campaign file in the repo (hard rule 9) beside the two key-leak tests in CI, plus the planted-PAN test that proves the sweep still bites.

> **Amended 2026-09-05 (WP55, `46-COUNTERPARTS.md` §4.2, §4.5):** `checkDesk` gained `desk.counterpart-script` — every script a desk carries has a non-empty `fallback`, rule ids unique within it, a trigger kind from the five, pressures in 0..1 and valid patterns (mirroring `describeScriptProblems` in `desk`; the kit stays `core`-only), and its injection sample carries the fifth kind. Two goldens now sit under `packages/desk/src/fixtures/`: `trace.desk-minimal.v1.json` (one seat, no script) and `trace.desk-counterpart-offline.v1.json` (two seats, the visitor on the `scripted-counterpart` brain — the merged stream, byte-identical, reproduced twice from one seed). The Workshop pack gained a `./testing` surface (its scripted plans) so the harness's scripted brains play a desk card.

> **Amended 2026-09-05 (WP58 stage B, `47-SERVICE-LINES.md` §4.3):** the kit gained `checkServiceLine(line, fixture?)` for every service line a manifest ships (`PackConformanceFixture.serviceLines`, keyed by line id; a line with no entry is checked with the defaults, `describeConformance` passing the pack id): every operation tiered with a `failureChance` in 0..1 and compilable `parameters`; `simulate` identical twice under identically seeded contexts with the clock and platform randomness replaced by throwing stubs (the desk check's device); every cassette entry replaying to itself twice and unrecorded arguments to nothing; the synthesised tool of a recorded line answering a `cassette-miss` under a throwing `fetch` on `globalThis`; a `live` block declaring a host with a purpose; a planted credential never in a `live.call` result or error, nor in a cassette. Proven in `checks/service-line.test.ts` against a tierless operation, a clock-reading simulation, a mute and a leaky live client, a broken cassette. `craftabot record`'s own test in the harness plants a key, records a stub line under its egress, reads the CORS checkpoint, and proves a line that embeds its key in a result is refused rather than redacted.

...
> **Amended 2026-09-06 (WP72, `61-LAST-DECKS.md` §4.1).** A sixth kind, `provider-fault { atTick, fault: 'timeout' | 'refusal' | 'garbage', count? }` — the one injection that is not world content: `isWorldInjection`/`splitInjections` beside the schema, `injectedWorld` handing a world its injections only, `runScenario` and the campaign runner handing the faults to the session (`SessionOptions.providerFaults`, through `runToCompletion`). A world never receives one; the desk's allowed-kinds default is unchanged.
...
> **Amended 2026-09-02 (stage C — WP44 closed).** (An earlier draft of this note said Phase J closed here; Phase J is WP43–WP46.) Gate: core 631, starter 683, governance 165, evals 105, harness 98, workbench 858; lint (34 tasks), build within budget (872 kB of 1465), e2e 151/151 (one new spec; a first full run dropped one load-sensitive spec, green on the rerun). `27-…` §8 item 13 carries the summary.

> **Amended 2026-09-05 (WP55 stage A, `46-COUNTERPARTS.md` §4.2):** `injectionSchema` gained a fifth kind, `{ kind: 'counterpart', scriptId }` — a scenario names which of a desk's `counterparts` sits across the desk, and the desk runtime seats that script (replaying its `opening`) when the injection lands, before the session starts, through the door `runScenario` already uses. The Playroom ignores it. `checkDesk`'s injection sample carries the kind, so `acceptedInjections` keeps its meaning; the Front Desk still lists `['heard']`.

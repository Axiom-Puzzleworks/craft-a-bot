...
> **Amended 2026-09-07 (WP73, `62-THE-TAIL.md` §4.3) — attempted again, still pending.** `npm run smoke:geap` with the `.env` on the machine WP73 was built on: `bad-token` on the Model Armor leg (the stored access token has expired again; Google's tokens live an hour) and no `gcloud` on `PATH` to mint one, so the evaluation leg never ran. The command above is unchanged and is the whole of the checkpoint; the browser half is the Evaluators page with the battery in. Whoever has `gcloud`: mint, run, paste the leg's three lines here with the date.

...
> **Amended 2026-09-07 (WP73, `62-THE-TAIL.md` §4.3) — the checkpoint is now one command, still pending.** `npm run smoke:azure` (`packages/packs/azure-content-safety/scripts/smoke.ts`, beside `smoke:geap`) takes it headless: with `AZURE_CONTENT_SAFETY_KEY` and `AZURE_CONTENT_SAFETY_ENDPOINT` in the environment it shields the known attack (must be detected), analyses a benign sentence (every category at 0), prints both latencies and the dated line for this note, and checks the key is absent from both payloads; without them it says how to get them and exits 0. **Attempted 2026-09-07 on the machine WP73 was built on: no Azure resource, no key — not taken.** The browser half (CORS from the Guard Rack, which decides `browserCapable`) remains the one click the note above describes. Whoever has a resource: run the command, paste its last two lines here with the date, and flip `browserCapable` if the Rack answers.
...
> **Amended 2026-09-12 (WP99, `83-…` §6.2.4, G65).** Bedrock is now a harness pack, as §2 predicted: **`@craftabot/pack-bedrock-guardrails`** — `ApplyGuardrail` on the shell with SigV4 signed in `sigv4.ts` (Node's `crypto`; the vault's one string is `accessKeyId:secretAccessKey[:sessionToken]`, read by the harness as `CRAFTABOT_CREDENTIAL_AWS_BEDROCK`), the content filters read as findings (`PROMPT_ATTACK` injection, the harms with the vendor's confidence), the anonymised text as `redactedText`, topics and words as policy violations; `browserCapable: false` on the service and on the component's connection, and a browser edition refuses to plug it in live with the reason (`browserRefusal` in `governance`, shown on the Guard Rack, which still fits it unplugged; the Guard brick's `guard-service-harness-only` warning when the screening is not offline). The natural third is here too: **`@craftabot/pack-lakera-guard`** — `/v2/guard` with a bearer token, the `breakdown` read by detector type; `browserCapable: 'checkpoint-pending'` on the connection, since whether the API answers a browser's preflight is a checkpoint nobody has taken. Both ship offline stand-ins that serve the fixture the config names, pass the shell's contract suite and the component and connection checks, and are installed by the harness's default host and by no edition. **Live checkpoints, pending:** `npm run smoke:bedrock` (`CRAFTABOT_CREDENTIAL_AWS_BEDROCK`, `AWS_BEDROCK_REGION`, `AWS_BEDROCK_GUARDRAIL_ID`) and `npm run smoke:lakera` (`LAKERA_GUARD_KEY`) — one command each, never in CI, recorded here when someone with a key runs them. The five shipped services' connections are declared in full: Model Armor with its filter `v3` and multimodal flag as config, streaming sanitisation recorded not applicable, and its 2026-09-01 checkpoint; OPA with its 2026-09-02 checkpoint; Azure `'checkpoint-pending'` (the key is still the blocker, `62-…` §4.3); the two local classifiers as `local` connections. `checkConnection` in the testkit runs for every component with a connection.

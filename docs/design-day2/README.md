...
## Where things stand (as of 2026-08-20 — see `18-…` §7 for the full log)

Phases A–D are closed (WP11–WP25): engine trust, the open brick contract, teaching-aid UX, the professional Workshop, and a full governance layer (policy cards, Safety Brick v2, three scenario cards) have all shipped. **Phase E (expansion era) is fully closed** (WP26–29 all done): WP26 (LLM Multi-Pack — three provider packs, persona cartridges, battery-bay UI, Compare view), WP27 (Monitor + Test Bench bricks), WP28 (the Workshop world pack), and WP29 (multi-agent core — `SessionGroup`, the Playroom's `forAgent` facade, group-altitude events, a minimal Workshop surface, built across seven gated stages per `23-…`). **Phase F (the ages 5–11 "Agent Builder" kit, WP30–34) is under way: WP31 (Radio brick + Robot Friends duo experience, including the ASI07 scenario) is done**, built across eight gated stages per `24-…`, **WP33's identity badge half (the Passport, and its Agent Card export) is done too**, sized down before it started and closed out in `14-…` §5.8's own dated amendment, **and WP30 is now fully done**: the Planner half (the brick, its live checklist, and its own eighth leaflet chapter) across five gated stages, and the If/Then half (the reflex mechanism, the brick, and its own ninth leaflet chapter) across four more, both closed out in `14-…` §5.1 and §5.2's own dated amendments; `18-…` §7's most recent entries (items 21–26) cover WP26's, WP29's, WP31's, WP33's and WP30's own close-outs and are worth reading before starting the next WP. WP32, WP33's remaining kit-line-packaging half, and WP34 remain — the kit-line half genuinely blocked on WP32 shipping real content to bundle (WP30's now done), not merely unstarted.

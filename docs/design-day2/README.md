...
| `29-GUARD-SHELL.md` | WP39's design of record (opened 2026-09-02, Phase I): the vendor-neutral hosted-guardrail contract `GuardrailService` in core, the shell `createHostedGuardrails`/`verdictForReading` in governance extracted from the Armour Brick, `pack-geap` put back on top with its golden trace byte-identical, the generic `workshop/guard` brick and `checkGuardrailService`; divergences from `26-…` §6.1 logged in §8 |

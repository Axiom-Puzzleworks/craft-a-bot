...
| `25-ARMOUR-BRICK.md` | WP35's design of record (opened 2026-09-01, Phase G): the Armour Brick — Google Cloud Model Armor as a hosted, Workshop-only safety brick (`geap/armor`) composed with the local governance floor; the credential/network seams it adds to the brick contract, the `guardrail.external` event, decisions D1–D5, and the five-stage implementation plan, each stage's divergences to be logged dated in §8 |
| `26-TARGET-DESIGN-V3.md` | **New (2026-09-02)** — the Day 3 target design: the safety proving ground. Where the code is at `8776fcb`, the gap register G1–G20, tenets 6–11, the vendor-neutral `GuardrailService` shell extracted from `pack-geap`, evaluators, scenarios, policy-as-code v2 + external PDP, telemetry sinks, declared egress, the `Storage`/folds move, the headless harness and campaigns, socket capacity for layered defence, the content store; non-goals (no training), divergences, risks, acceptance |
| `27-DAY3-ROADMAP.md` | **New (2026-09-02)** — the phased plan to `26-…`: Phases H–L, WP36–WP52, each with DoD, size and the gaps it retires; dependency sketch; build discipline; controls adopted from `19-…`; every carried-in follow-up mapped to a WP; §8 session-sized next steps (WP36 first) |
| `34-CONTENT-STORE.md` | WP46's design of record (opened and closed 2026-09-02, Phase J): one store for authored policy cards, assertion cards, scenarios and campaigns under the reserved `local` pack, on all three stores and in the harness's `content/` directory; kit files carry the cards a bot fits; save on four Workshop screens and the Kit's gated picker; divergences in §7 and the stage notes |
| `33-POLICY-V2-PDP.md` | WP45's design of record (opened and closed 2026-09-02, Phase J): six more predicate leaves with a bounded pattern subset, the world's own questions on the guardrail context, progress declared by the world's actions, the PDP input document on every screen request, OPA as a guard service with its live checkpoint taken; divergences in §7 and the stage notes |
| `32-SCENARIOS.md` | WP44's design of record (opened and closed 2026-09-02, Phase J): a scenario as data — a goal card plus tags, injections, expectations and plans; `WorldInstance.inject?` and the Playroom's four doors; the four governance cards as scenarios; `runScenario`; the JSONL corpus importer and scenario pack files; campaigns by `scenarioId`; the by-tag table; `craftabot scenarios`; the Scenario Library; divergences in §7 and the stage notes |
| `31-EVALUATORS.md` | WP43's design of record (opened and closed 2026-09-02, Phase J): one contract for what judges a trace — `Evaluator` and the stored `EvaluationRecord`, every assertion card as an evaluator with real usage, the rubric judge as a pack, campaign gates on verdicts, the Evaluators page and the Run Lab's inspector, the Monitor Judge, `craftabot evaluate` and `checkEvaluator`; divergences in §8 |
| `30-SECOND-VENDORS.md` | WP42's design of record (opened and closed 2026-09-02, Phase I): the written vendor comparison and choice, `@craftabot/pack-guard-local` and `@craftabot/pack-azure-content-safety` as services and nothing else, the Guard Rack, the baseline's two stacks, and the Azure live checkpoint recorded as not yet taken |
| `29-GUARD-SHELL.md` | WP39's design of record (opened and closed 2026-09-02, Phase I): the vendor-neutral hosted-guardrail contract `GuardrailService` in core, the shell `createHostedGuardrails`/`verdictForReading` in governance extracted from the Armour Brick, `pack-geap` put back on top with its golden trace byte-identical, the generic `workshop/guard` brick and `checkGuardrailService`; divergences from `26-…` §6.1 logged in §8 |
| `28-CAMPAIGNS.md` | WP38's design of record (opened and closed 2026-09-02, Phase H): campaigns — scenarios × builds × guards × brains × seeds with gates, the adversary brain tier promoted from the scenario tests to content, the `CampaignReport` and its markdown/JUnit/SARIF renderings, `campaigns/injection-baseline.json` and the CI job that fails when a guard is removed, `craftabot campaign`, and `/workshop/campaigns`; each divergence from `26-…` §6.9 logged in §8 |

## Where things stand (as of 2026-08-21 — see `18-…` §7 for the full log)

> **Amended 2026-09-01:** Phase G opened with WP35 (`25-ARMOUR-BRICK.md`) — the first hosted-control brick. Nothing below is changed by it; Phases A–F remain closed.
>
> **Amended 2026-09-02:** Phase G is closed (WP35 done, `18-…` §7 item 32). The fresh planning pass that `18-…`'s exhausted plan asked for is done: `26-TARGET-DESIGN-V3.md` is the target design and `27-DAY3-ROADMAP.md` the forward plan (Phases H–L, WP36–WP52). Nothing in it is built; WP36 is next.

...
Phases A–D are closed (WP11–WP25): engine trust, the open brick contract, teaching-aid UX, the professional Workshop, and a full governance layer (policy cards, Safety Brick v2, three scenario cards) have all shipped. **Phase E (expansion era) is fully closed** (WP26–29 all done): WP26 (LLM Multi-Pack — three provider packs, persona cartridges, battery-bay UI, Compare view), WP27 (Monitor + Test Bench bricks), WP28 (the Workshop world pack), and WP29 (multi-agent core — `SessionGroup`, the Playroom's `forAgent` facade, group-altitude events, a minimal Workshop surface, built across seven gated stages per `23-…`). **Phase F (the ages 5–11 "Agent Builder" kit, WP30–34) is under way, with WP30, WP31, WP32 and WP34 now all fully done**: WP31 (Radio brick + Robot Friends duo experience, including the ASI07 scenario), built across eight gated stages per `24-…`; WP30 (the Planner half — the brick, its live checklist, and its own eighth leaflet chapter — across five gated stages, and the If/Then half — the reflex mechanism, the brick, and its own ninth leaflet chapter — across four more), both closed out in `14-…` §5.1 and §5.2's own dated amendments; WP32 (the Librarian and Connector bricks, across four gated stages — the two bricks, the Librarian's own tenth leaflet chapter on a genuinely new goal card, and Connector's own confused-deputy mini-scenario shipped as a side quest), closed out in `14-…` §5.6's own dated amendment; and **WP34** (Workshop maturity, across four gated stages — `/telemetry`, `/incidents`, `/safety-case`, and the Audit Centre at `/export`, none needing a core change), closed out across `17-…` §4.7–§4.10's own dated amendments. **WP33's identity badge half (the Passport, and its Agent Card export) is done too**, sized down before it started and closed out in `14-…` §5.8's own dated amendment; `18-…` §7's most recent entries (items 21–28) cover WP26's, WP29's, WP31's, WP33's, WP30's, WP32's and WP34's own close-outs and are worth reading before starting the next WP. WP33's remaining kit-line-packaging half is the only piece left in Phase F — no longer blocked on missing content now that WP30 and WP32 have both shipped, but not yet started.

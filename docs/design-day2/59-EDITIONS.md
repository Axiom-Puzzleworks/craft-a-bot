...
6. **`vite preview` has one SPA fallback.** A static host needs a rewrite per folder (`/simulator/*` → `/simulator/index.html`); `vite preview` rewrites to the root `index.html` only, so a deep link into an edition's folder under preview lands in the `full` build. **Decided, then corrected at stage C:** `vite preview` does not even serve a folder's own `index.html` for `/playground/` — its fallback answers with the `full` document, so nothing of an edition could be reached under it. The smoke specs run against `scripts/serve-site.mjs` (`npm run serve:site`), a static host as `docs/publishing.md` describes one: the `build/` tree with one SPA fallback per folder. Deep links work there and on any host with the rule, which is every static host worth the name.
...
> **Amended 2026-09-07 (WP73, `62-THE-TAIL.md` stage A):** the fourteen wave 2 placeholders are inlined as markup (`lib/assets/instruments.ts`, `finishes.ts`, the box), which put the Playground edition 1 kB over and the Simulator 1 kB under. Both raised by 15,000 bytes — `simulator` 1,445,000, `playground` 1,480,000 — a deliberate step for a known cause, as the risk in §7 anticipates; `workshop` and `full` unchanged.
...
`playwright.editions.config.ts`: one `webServer` (`npm run build:editions && node scripts/serve-site.mjs`), three projects — `simulator`, `workshop`, `playground` — each with `baseURL` at its folder and `testMatch` on `e2e/editions/<edition>.spec.ts`; the default config ignores `e2e/editions/`. The smoke specs enter at the folder root and navigate: the Simulator's first run (new bot, build, GO, an end card) with the leak gate (a fitted key appears nowhere on the page or in a download); the Workshop's Run Lab (a run played, opened in the Run Lab); the Playground's Advice Desk (the Playground page, the Advice Desk's case). A fourth check per edition: a route outside the allow-list renders the edition's page. CI gains an `editions` job: `build:editions` (the three budgets), `e2e:editions`, and the three folders uploaded as one artefact.
...
>
> **Stage C landed 2026-09-06 — WP69 closed.** `playwright.editions.config.ts` with three projects, each `baseURL` at its folder; `e2e/editions/*.spec.ts` — the Simulator's first run to its end card with the leak gate (a battery fitted through Settings, the page and the exported kit file carrying no key), the door as a link to `/workshop/` and no rocker, the shelf's *In the Playground →*, a Workshop route rendering the not-in-this-box page with its link; the Workshop's door open by default, a run opened in the Run Lab under `/workshop/workshop/runs/<id>`, the Playground route not in the box and the rest in it; the Playground's Advice Desk generating a case under `/playground/`, every route in the box, the rocker present. The finding of the stage (§2 item 6): `vite preview` answers `/playground/` with the `full` build's document, so the specs run against `scripts/serve-site.mjs`, a static host with one SPA fallback per folder — which is also what `docs/publishing.md` §2 asks a host for, and what a deployment check can use. The specs navigate inside the app through an anchor the router intercepts (`clientGoto`), so a deep link is the host's job and not the spec's. CI gains the `editions` job: the app's packages built once, `build:editions` (the three budgets), `e2e:editions`, the three folders uploaded as `site-editions`. `docs/publishing.md`: the folders and the sections, the rewrite rule per host (Netlify, Vercel, Cloudflare Pages, nginx, GitHub Pages), the login as a hosting rule (D3), keys per origin and the subdomain option, the deployment check. Close-out: `42-…` §3 and §8 items 57–59, `41-…` §12, `01-…` §5 and §8, `15-…` §2, `CLAUDE.md`, `README.md`. Gate: root lint, every workspace's tests, `full` built within its budget with schemas matching, the evals baseline, the default e2e and the visual set, the editions suite (5 specs over three folders).
...
| `configure(config)` | Stored in `extra.config`; nothing reads it yet (the Radio brick's channel, if ever fitted to a desk, is WP55's). > **Amended 2026-09-10 (WP78):** read now — and too late for a case's truth, so `create(layoutId, { config })` (`WorldCreateOptions.config`, additive) seeds `state.config` first and hands the config to the layout's `case(random, config)`; a reset and a restore rebuild with it; `configure` still merges over it. The Lending Desk reads `config.knobs` as its policy (`52-…` §4.2's note). |
...
> **Amended 2026-09-11 (WP81, `70-CONTEXT-AND-ONTOLOGY.md` §4).** The runtime composes the case by the rung of the context ladder the create-time `config.context` names: `minimal` keeps the records the queue names, `case-file` is as today, `relational` and `ontology` add what `DeskWorldSpec.context(level, generated, spec)` makes — never a special-category record — with `include`/`exclude` by kind, a token budget over the added records' text, and the `brief` or `sense` delivery (the desk brief's text, or a *Handed over as context* line the observation carries whatever senses are on). `DeskState.contextRecordIds` names what was added, present only when a context was configured, so every trace written before keeps its bytes. `checkDesk` gains `desk.context-superset` and `desk.context-classification`.
>
...
> **Stage B landed 2026-09-05.** `@craftabot/desk` (`packages/desk`, `core` + `zod` only, held by its own ESLint block) with `createDeskWorld` supplying every door of §4.4; the Front Desk rewritten on it — same ids, same card, same e2e, a hundred lines where stage A's hand-written version was two hundred and fifty; `trace.desk-minimal.v1.json` (45 events, 13 types) over the runtime's own test desk as the byte-stability oracle, parsed clean through `engineEventSchema`; `create(layoutId, { random? })` in core with the session and the group passing their own stream, proven by `world-create-options.test.ts`. Three things diverge from §4.4 as first written, all towards less mechanism. (1) **`DeskActionSpec` takes a Zod schema, not a JSON Schema**: the advertised `parameters` are derived with `z.toJSONSchema` and the arguments parsed by the same schema — exactly what both grid worlds' `defineAction` does — so the runtime needs no JSON-Schema validator of its own, and the testkit's Ajv check over the advertised schema remains the independent proof. (2) **The runtime's own senses and `say` are declared by `kind`**, not inferred from an id: `{ id, kind: 'conversation' | 'case-file' | 'queue' }` and `{ id, kind: 'say' }`; a desk names them, the runtime renders them; a custom sense supplies `reveal`. (3) **A `radio` injection speaks as its `fromName`** (a `system` line on the channel), not as "Desk". Also: a desk's seed is one draw from the session's `random` (§4.4's seam), so the dice tool and the desk share a stream without the desk consuming it; `reset` rebuilds from the kept seed. `DeskWorldDefinition` carries `spec` for `checkDesk` (stage C) to read. Gate as `42-…` §8 item 5 records.

> **Stage C landed 2026-09-05 — WP53 closed.** `DeskView` proper (§4.7): the three panes over `DeskWorldState` with a label and a glyph on every lane (never colour alone), the classification badge with a title that says why the record is shown, the alerts strip, the "FOR SIMULATION ONLY" strip, the transcript inside a `role="log"` live region *wrapping* the list (axe refused `role="log"` on the `<ol>` itself: a list with another role has no list items as far as a reader is concerned), the Workshop's token layer densifying it untouched. Two things §4.7 promised are not here and are recorded: rows are not focusable — svelte-check refuses `tabindex` on non-interactive elements, and a row that does nothing when focused is not a keyboard path; they become focusable when WP57 makes them interactive (a queue item that scrolls the case file). And `contrast.test.ts` gained no rows: every pair the Desk uses (ink and muted ink on cream and on paper; red as a non-text border on cream) was already held. `checkDesk` (§4.8) in the testkit, run by `describeConformance` from `PackConformanceFixture.desks`; the Front Desk under it with its `acceptedInjections: ['heard']`, its scripts and its illegal calls, and the three failing desks of the DoD proven in `checks/desk.test.ts` against hand-written worlds (the kit depends on `core` only). The a11y suite audits the Desk mid-run, at the end card, and in the Run Lab. `buildDeskBotAndGo` in `support.ts` is the shared way an e2e puts a bot on the Front Desk. Every acceptance criterion in §11 holds; the gate is `42-…` §8 item 6's.

...
> **Amended 2026-09-12 (WP96, built).** The two verdict kinds are applied, not only carried. **`redact`**: `runGuardrailChain` surfaces the first `redact` allow on a chain as `ChainOutcome.redaction` (`{ guardrailId, redactedText, finding? }` — later redactors see the original and are recorded on their own rows); the session rewrites an action's string `text` with it before the call runs — `say`, on every desk — and `action.performed` carries the rewritten `arguments` and a `redacted: { guardrailId, finding? }` field, while the bot's own words stay on the `decision` event; the trace digest covers the rewritten line because it is the event's bytes. A call with no `text` runs as decided, the verdict still on its `guardrail.checked`. At a stage boundary the workflow runtime applies it to the stage value (WP95). The shell (`governance/hosted/verdict.ts`) turns any allow into a `redact` when the reading carries `redactedText`, with the matched `sensitive-data` finding on it; a `note` disposition is now an **`annotate`** with the fired finding — `{ allow: true, verdictKind: 'annotate', note, finding }` — so `pack-geap`'s note expectation and its `guardrail.checked` rows gained the two fields, and nothing else on its golden trace moved. Model Armor's Sensitive Data Protection is the first `redact` component: its reading already carried `deidentifyResult.data.text`; `armorServiceConfigSchema.offlineFixture` lets the stand-in serve `sdp-deidentified` so a redaction is rehearsed offline. The Desk's transcript marks a redacted agent line (`desk-line-{seq}-redacted`) with the guard and the finding, read off the run's `action.performed` events by tick; `fs-advice/pii-contained` cites lines redacted before they were said. `annotate` never changes an outcome — `packages/harness/src/annotate-identity.test.ts` runs the lending baseline with an annotating component at every loop point beside the translated guards and compares every cell's outcome, assertions and evaluations.
>

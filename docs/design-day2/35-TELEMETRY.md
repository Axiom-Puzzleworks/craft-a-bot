...
> **Amended 2026-09-06 (WP65, `55-PRINCIPAL.md` §4.3).** The OTel mapping carries the principal. The `invoke_agent` root and every `execute_tool` span gain `gen_ai.agent.id` (the record's agent id) and, when the run's `run.started` names a principal, `craft_a_bot.principal.kind`, `.id`, `.name` and `.chain` (the chain rendered `kind:id`, innermost first, joined ` <- `: `agent:<id> <- person:<id>`); the `invoke_group` root carries the group's own from `group.started`. The conventions have `gen_ai.agent.id`; a person or service behind a run has no convention, so the rest are `craft_a_bot.*`. A run that names none gains only the agent id.

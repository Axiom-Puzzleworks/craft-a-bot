...
> **Amended 2026-09-06 (WP65, `55-PRINCIPAL.md` §4.5).** The header names who started the run (*started by <name or id>*, the chain as the tooltip) when `run.started` carries a principal; the inspector shows **Chain** — who, for whom, who approved, which rules passed — above the payload of an action row with an attestation and of an approval row with a `by`. The Settings page's Workshop preferences gain *Your name, on the trace*.
>
> **Amended 2026-09-06 (WP66, `54-FORK-EXPLAIN.md` §4.5).** Causal row linking and "Fork from this tick" are built. The inspector's **Explain** checkbox renders governance's `decisionExplanation` for the selected row's decision — saw, prompt, offered, chose, checked, asked, did, had in hand — each line a link to its row, and lights every related row of the timeline while it is open. **Fork from this tick** in the header forks a stored run from the scrubber's completed turn through the app's own session path (the demo brain resumed where the origin's was, or the cartridge's provider), stores the fork with `forkedFrom`, and opens Compare synced from the fork point. No overrides in the Workshop — a counterfactual build is `craftabot fork --kit`. "Re-run with same seed" remains unbuilt.
>
...
> **Amended 2026-09-06 (WP70, `58-EVIDENCE-STORE.md` §4.5).** `/workshop/evidence` joins the rail between *Sinks* and *Playground*: the shared evidence store's cards (config, egress, the token battery), a push over what this browser holds and a pull that verifies every item's digest before it can be imported — a pulled bundle landing in the Run Browser as an imported file does. The Audit Centre gains *Push to evidence store* beside its download and Campaigns a *Push* on a saved report, both only once a store is configured; nothing here requires the store.
>
> **Amended 2026-09-06 (WP68, `57-HARNESS-AT-SCALE.md` §4.5).** The import takes the file sink's JSONL beside a trace file — a run the harness may still be writing (`craftabot run --sink telemetry/file`): the record provisional and `IN_PROGRESS` until the file has a `run.finished` line, the Run Lab's header saying *still going — re-import the sink file to catch up*, and a re-import replacing the run's events with the file's. This is the harness half of live trailing `37-…` §7 left recorded: the file is the stream, a re-import the refresh, no socket.
>
> **Amended 2026-09-06 (WP66).** Compare takes a third parameter, `from=<tick>`: when `b`'s record says it was forked from `a` after that tick, `b`'s panel folds `a`'s events through the tick before its own, the scrubber opens at the fork point and the page says *forked after turn n*. The Run Lab's "Fork from this tick" opens it this way; two unrelated runs ignore the parameter.

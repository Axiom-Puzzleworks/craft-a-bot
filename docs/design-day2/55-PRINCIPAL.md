...
> **Beside WP65, 2026-09-06.** `main`'s CI had been red since the evening of 2026-09-05 on a core test that raced the clock (`12-…` D22: the engine's resume tests scripted forty 1 ms ticks and a fast Linux runner finished the run before the second pause). Fixed in this PR so it could merge on a green check; nothing of WP65's is in the fix.

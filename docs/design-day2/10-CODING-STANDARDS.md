...
> This file is a verbatim copy of `docs/design/10-CODING-STANDARDS.md` carried into the standalone Day 2 set; only this banner has been added.
...
7. **Nothing real, ever** — hard rule 9 (`41-TARGET-DESIGN-V4.md` tenet 15, §6.13; built by WP54, `45-TRUTH-SYNTHETIC.md` §4.5–§4.6, 2026-09-05). Every case file, customer, transaction, document, cassette and corpus row in the repo is synthetic and generated from a seed through the primitives in `@craftabot/desk`; a hand-written case file is refused in review. The floor is `checkSynthetic` in `pack-testkit` and its sweep (`synthetic-sweep.test.ts`, beside the key-leak tests in the same CI job) over every fixture, cassette, scenario, baseline, corpus and campaign file: a Luhn-valid card number with an issuer prefix, a valid IBAN, a sort code outside `99-` under a key naming it, an issuable NI number, an email on an unreserved domain, a UK telephone number outside the drama ranges — any of these fails the build, and the fix is to regenerate, never to relax the shape.

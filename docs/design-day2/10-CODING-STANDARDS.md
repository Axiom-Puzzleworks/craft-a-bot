...
7. **Nothing real, ever** — hard rule 9, a placeholder pending WP54 (added 2026-09-05; `41-TARGET-DESIGN-V4.md` tenet 15, §6.13). Every case file, customer, transaction, document, cassette and corpus row in the repo is synthetic and generated from a seed through `random()`; a hand-written case file is refused in review. WP54 lands `checkSynthetic` in `pack-testkit` and the CI sweep beside the key-leak test, at which point the sweep is the floor and this item stops being a placeholder.

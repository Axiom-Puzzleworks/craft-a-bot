...
> **Status (2026-09-10): built in full — Phases M–Q, WP53–WP73, every row of `42-…` §3 ✅. Superseded as the target by `64-TARGET-DESIGN-V5.md`, which extends this document (every contract in §6 stands) and whose plan is `65-DAY5-ROADMAP.md`.**
>
...
> **Amended 2026-09-06 (WP72, `61-LAST-DECKS.md`).** Both decks are built. Complaints & redress is a second world in the Advice Desk's pack (`fs-advice/the-complaints-desk`, `purpose: 'complaints'`) rather than the Advice Desk's own world, since a spec has one purpose; DISP's timescales are ticks held in truth; redress is gated by approval. The operational incident's seam is `provider-fault { atTick, fault, count? }` (`fault`, since `kind` is the union's discriminator), retried through by the session so the run continues and what follows can be judged; *Fallback* and *told plainly* live in `fs-bank` so every desk gates on one id. `61-…` §8 records each.
...
| §6.5.1: each line "`simulate`s over the bank's state" and an irreversible operation acts (WP59, `48-…` §8) | A line answers from the bank in `worldState.extra` and returns the mutation as `data`; the desk's action writes it | `47-…` §3: a line reads the world, it does not act in it — the trace attributes the act to the bot's decision |
| §6.5.1: "a synthetic 'protected-characteristic proxy' flag set" (WP59) | Six opaque flags `proxy-a` … `proxy-f`, one to three per customer, held in truth | Never a real characteristic, even a synthetic one, in a fixture (tenet 15) |
| §6.7: the bank's control-map rows are registered content (WP59) | Shipped as data under `fs-bank/src/controls/`, typed locally; registered by WP67 | The `controlMaps` lane and `checkControlMap` are WP67's |
| `42-…` WP59 stage C: "the bank's lines on a static `Boundary`" via `boundaryMapFor` | A hand-built `BoundaryMap` with `service-line` nodes around an empty agent | `boundaryMapFor` needs a spec; the Playground page shows the bank, not a bot |
| §6.5 names the bank's note `45-FS-BANK.md` and the Advice Desk's `46-FS-ADVICE.md` | `48-FS-BANK.md` and `49-FS-ADVICE.md` | `45-`, `46-` and `47-` went to WP54's, WP55's and WP58's notes, which landed first |
| §6.5.2: campaign brains `scripted-optimal, scripted-counterpart(pushy), scripted-counterpart(injecting)` (WP60, `49-…` §8) | Brains `scripted-optimal` and `scripted-adversary`; the persona is the layout's, chosen by the card; two sanity gates on the `pushy` and `injecting` scenarios | `scripted-counterpart` is a seat's brain and the runner refuses it for a cell (`46-…` §4.5); the persona was always inside the runtime |
| §6.5.2: "advice only when the build's configuration allows advice" (WP60) | The *card* sets the boundary (`advise-…`/`guide-…`); `boundary-held` reads `run.goalCardId` | Nothing reads a world's `configure`; a card is content, a config lane a mechanism |
| §6.5.2: eight deterministic evaluators and four rubric *configs* (WP60) | Nine (with `execution-approved`) and four rubric *evaluators* under their own ids | A campaign keys verdicts by evaluator id; a cell resolves its own approvals, so "never without approval" is a trace property |
| `42-…` WP60 stage D: "a desk's cards in the Kit's rack when the box is open" | `audience: 'workshop'` — the Workshop door is the gate, as for the Front Desk | No per-box open state exists; inventing one is a Kit mechanism for a Kit WP |
| Implicit in `evals`: scripted plans are the starter pack's (WP60) | `PlanSource` — `chainPlans`, `plans?` on the runners; the host composes its packs' `/testing` plans | The only way a desk card runs in a campaign without `evals` importing every pack |
| §6.6: `labelSemantics?: { kind: 'confusion'; positive: string[]; … }` (WP61, `50-…` §8) | Four named labels, one per matrix cell | A single `label` per result is the contract; the evaluator says which label is which cell |
| §6.6: the report "carries `labels` per slice"; per-case metrics "join `scoreRun`" (WP61) | A `summary` block folded once (slices, matrices, cohorts, obligations, cases); `caseMetrics` on the cell from the world's `metrics` | One fold, three renderings; `scoreRun` knows no world |
| §6.6: `parity: { metric \| derived \| labelRate, … }` (WP61) | `parity.of` as a discriminated union over the existing measures, `matched` on the gate and the verdict | One measure grammar for five gate kinds |
| §6.6: drift "gains the evaluation records and the campaign reports as inputs" (WP61) | `TelemetryExtras` and a structural `DriftReportLike` in governance; `seriesThreshold` beside the two thresholds | `evals` depends on `governance`; the reverse would be a cycle |
| §6.5.3: `alert-decision` "per alert: tp \| fp \| tn \| fn" (WP62, `51-…` §8) | The case's focal alert, one label per result; `queue-decisions` scores the rest | One label per result is the contract (WP61); a campaign's matrix is over cases |
| §6.5.3: the stress deck "under a token budget" (WP62) | A tick budget, `maxTicks: 8` over twenty alerts | No token lever reaches a campaign file; the triage is the same |
| §6.1: a desk's handlers get `reveal`, `find`, `decide`, `alert`, `line`; predicates read the state (WP62) | `ctx.open` and `test(state, truth)` — the two contract changes the second desk needed | A queue's `in-progress` status was unreachable; a rule about what was *so* needs the truth the runtime holds |
| §6.5.4: *Cohort-blind* "over `GuardrailContext.messages`" (WP63, `52-…` §2) | A `prompt-contains` leaf in `core`, evaluated in `governance` | No leaf read the composed prompt; a pack guardrail would break hard rule 4 |
| §6.5.4: *Reasons are real* as `argument-contains` each of the decision's reasons (WP63) | The desk refuses a reason the decision never used; the card guards the order; `explanation-faithful` scores it | A card is static data and cannot name a run's reasons |
| §6.9: `decisionExplanation` with `reasonsUsed` inside it (WP63 lands the first field) | `reasonsUsed(events, decisionEventId)` alone, in `governance/reports` | The fold's first consumer is a conduct check; WP66 grows it |
| §6.9: the fork "seeds `random` from the recorded stream position" (WP66, `54-…` §8) | The caller's `random`; identity proved over worlds that draw nothing after creation | The stream's position is not on the trace |
| §6.9: a fork "reproduces the original's events after *n* byte-identically" (WP66) | `{ type, tick, payload }` identical after *n*; ids, run id and timestamps the fork's own | A fork is a new run |
| §6.9: the fork "rebuilds the world by replaying `world.changed` through `perform`" (WP66) | `restore(snapshot)` where a world has it; else the recorded *calls* replayed and checked against the snapshot | A `world.changed` carries a state, not a call |
| §6.9: `overrides.counterpart` (WP66) | `overrides.world` | A counterpart is seated inside a world; the world is the general door |
| §6.3 / `42-…` §3: "persona cartridges for every script in the bank's library over any provider" (WP64, `56-…` §8) | The persona is the seat's `personality`; the cartridge is any installed one | A cartridge is a provider and a model; the persona is already on the seat |
| §6.3: `counterpart: { tier, script }` on a campaign, per cell (WP64) | `{ tier, cartridgeId?, maxRounds? }`, one per campaign; the script is the layout's | Two tiers are two instruments; the card chooses the case and its person |
...
| `42-…` row: "the *told plainly* evaluator" per desk (WP72) | One evaluator in `fs-bank`, gated by every desk's campaign | Every desk requires the bank; one id, one meaning |
| §6.12: the screenshots "committed" for the build platform (WP71) | A baseline per platform — Windows from the build machine, Linux from CI's `visual` job | Playwright will not compare across platforms |
| §6.14: `importKitFile`'s "`version-mismatch` path, one message wider" (WP69, `59-…` §8) | The `missing-packs` message widened in the app; `core` unchanged | A pack from another section is missing, not mismatched |
| §6.14: "the key-leak test runs over all three folders" (WP69) | The Simulator's smoke spec carries the leak gate through the UI under its base | The folders hold no key by construction; the gate that can fail is the UI's |
| §6.14: the Workshop's section at `/workshop` (WP69) | Its screens at `/workshop/workshop/…` | File-based routes; a route alias per screen is not worth its weight |
| §6.14: "a Playwright project per edition … with `base` set" (WP69) | Three projects against `scripts/serve-site.mjs`, a static host with one fallback per folder, not `vite preview` | `vite preview` answers a folder's root with the `full` document |
| §6.11: "new package `@craftabot/evidence` (core only)" holds the contract (WP70, `58-…` §8) | The contract in `core` (`types/evidence-store.ts`, `schemas/evidence.ts`); the package holds the stores | `PackManifest.evidenceStores` and the testkit name the type without the package — the sink's precedent |
| §6.11: "each carrying its own digest" (WP70) | One digest rule over the canonical payload; a bundle's own digest verified besides on import | Two kinds have no digest of their own |
| §6.11: anon key and token both "in the vault" (WP70) | The token in the vault; the anon key in the store's config, planted in the leak sweep all the same | One credential per declaration; the anon key is publishable by design |
| §6.11: the note is `49-EVIDENCE-STORE.md` (`42-…` row) | `58-EVIDENCE-STORE.md` | Notes are numbered in the order written |
| §6.10 / G35: "harness-side live trailing through the file sink" (WP68) | The Run Browser imports the sink's JSONL, a run in progress included; re-import to catch up | No server in the browser; the file is the stream |
| §6.5.6: the breaker "on `tipped-off`/`unsuitable`" (WP64) | On any evaluator's named labels or its fail; the Advice Desk's on `suitability-complete` failing | Truth never reaches a chokepoint mid-run |
| §6.8: "a spawned sub-run (`parentRunId`) chains" (WP65, `55-…` §8) | A group member chains through `onBehalfOf`; a fork carries the forker's principal | The only sub-runs are group members; a fork is a new run by whoever forked it |
| §6.7: *validated by* — "a campaign run under a principal other than the builder's" (WP65) | The principals of the evaluated runs, with a note that the builder is not recorded | No record names a builder |
| §6.8: `execute_tool` spans get "`gen_ai.agent.id`-style attributes" (WP65) | `gen_ai.agent.id` and a rendered `craft_a_bot.principal.chain` on the root and on `execute_tool` | The conventions have the agent id; the chain has no convention |
| §6.9 / `42-…` §3: the Run Lab's fork "with synced scrubbing" over any override (WP66) | The Workshop forks without overrides and grants any approval; a counterfactual build is `craftabot fork --kit` | Editing a fork's bot in the Spec Lab is its own WP |
| §6.6: a campaign's cells "per seed" (WP63 stage D, `52-…` §2 item 4) | The cell's seed now reaches the world through the harness's clock and `injectedWorld` | It reached only the brain's noise before; every cell saw one customer and a parity gate had one cohort |
| §6.7: `ControlMap` "in `governance/reports`" (WP67, `53-…` §2) | The type in `core`; the generic rows and the fold in `governance` | `PackManifest` cannot name a governance type |
| §6.7: "every row's evidence must resolve" (WP67) | A row may be `pending` with no evidence, naming its WP; every other row resolves | Two of the bank's rows cite WP72's content; a dangling id is refused, a pending row is honest |
| §6.7: `assurancePackFor(agentId, storage, registry)` (WP67) | A pure `assurancePackFor(input)` and `assurancePackFromStorage(agentId, storage, registry)` | The fold is testable without a store; the storage wrapper is the two hosts' shared path |
| §6.7: the pack's "validated by" and "who acted" (WP67) | Present, saying *not recorded in this build* with the WP | WP65 is unbuilt; the document's shape should not change when it lands |
| §6.5.4: a matched pair "one seed apart" (WP63) | The pair's side from the layout's first draw; four seeds cover both | A layout takes a `random`, not a seed; the finances are a template either way |

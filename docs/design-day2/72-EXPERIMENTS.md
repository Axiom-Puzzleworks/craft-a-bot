...
**Levels.** A `guard` level is a guard id in the template (**amended 2026-09-12, WP97:** or a stack id no template guard has — the campaign gets `{ id, fit: [], stack: id }` and the runner resolves it; `AnalyseOptions.stacks` joins such a stack's `controls` to the effects' `controlIds`); a `brain` level a brain id; a `context` level a rung (`minimal` … `ontology`) or a template context's id; an `executors` level a workflow configuration id (every build gets it as `overrides.configuration`); a `knob` level a value of `factor.knob` (every build gets it as `overrides.knobs[knob]`, parsed as the sweep parses one). The campaign for a combination is `<experiment>--<axis>=<level>--…`, its seeds the design's seeds × replicates (`seed × replicates + r`), its one gate always passing (a measurement, not a judgment).

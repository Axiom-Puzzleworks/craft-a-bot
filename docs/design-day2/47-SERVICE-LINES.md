...
> **Stage C landed 2026-09-05 — WP58 closed.** The bench's Connector picker: `serviceLines` from `registry.listServiceLines()` threaded through `BrickPanel` and `panel-props` into `SchemaPanel`'s `catalogue('serviceLines')`, so the `serviceId` choice lists every line an installed pack ships by name and description; `build-checks.spec.ts` fits a Connector by the keyboard lift, sees both starter lines offered, picks Open-Meteo and reads `starter/open-meteo` back. The live checkpoint and the shipped cassette landed in stage B (its note); nothing was pending. `42-…` §3's rows and §8 items 16–18 (the `48-`/`49-` renumbering), `41-…` §12's four rows, `CLAUDE.md` (the table row, the workflow, `record` in the commands), `README.md`. **WP58's DoD, checked:** the WP32 confused-deputy scenario byte-identical before and after (`trace.confused-deputy.v1.json`, recorded from the pre-move code); a cassette replays twice identically and a miss is on the trace as `error.kind: 'cassette-miss'` with a throwing `fetch` on `globalThis` for the whole run (`open-meteo-replay.test.ts`, `checkServiceLine`'s `no-live-in-replay`); `checkServiceLine` rejects an operation without a tier and a `live` without egress (`checks/service-line.test.ts`); the live checkpoint against Open-Meteo taken 2026-09-05 — one host allowed, nothing refused, 36–355 ms, CORS open, `browserCapable: true`. G27 retired. Recorded for the Phase M exit review: a line's bare id lands in the wire name unchanged (`connector_open-meteo_forecast` carries a hyphen, which the providers' function-name grammars accept — a line id with any other punctuation would not), and `describeFitted` shows another pack's line by id (§8). Gate: root lint clean, every suite green with core's thresholds, build within budget with the schema check, default e2e 169/169 (one new spec), visual 3/3, baseline campaign with no regressions, all four goldens byte-identical.

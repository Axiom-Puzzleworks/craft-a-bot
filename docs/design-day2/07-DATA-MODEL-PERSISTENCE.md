...
> **Amended 2026-09-07 (UX-15, `docs/manual/UX-AND-GAPS.md`).** `RunRecord.abandonedAt?` — when a person marked a run left `IN_PROGRESS` as abandoned. A closed tab writes no `run.finished`, so the record's `outcome` stays what the trace says (`IN_PROGRESS`, honestly); the Run Browser offers to mark such runs, shows them as ABANDONED (`displayOutcome` in `lib/workshop/run-filter.ts`) and filters on that, and the live run is never offered. Nothing else changes: the field is optional, every record written before it parses, and no report reads it yet.

> **Amended 2026-09-10 (WP79, `69-WORKFLOWS.md` §4).** A fourth exported artefact beside the trace, the bundle and the campaign report: the **workflow run** (`WorkflowRun`, `docs/schemas/workflow-run.schema.json`) — one journey over one work item as its stage records (executor as data, input and output digests with the value when small, guard tally, the agent run's id when a bot did it, the approval when a person did, status and finding), the ids of every agent run it made, its own events (`stage.*` for the non-agent stages and the `action.performed`, `approval.*` and `tool.executed` they wrote) and a SHA-256 over the stage records' canonical JSON. Every agent run inside it is an ordinary run in the store — its record, events, summary and trace file exactly as `run` writes them — so nothing about a run changes; the harness writes the workflow run beside them as `<out>/workflows/<id>/workflow-run.json`. The browser stores none yet (WP86's Pipeline view is where it lands).
>
> **Amended 2026-09-07, later (NEW-3, `docs/manual/UX-AND-GAPS.md` §3).** `GroupRunRecord.abandonedAt?` joins the run's, and `core` now exports the one definition every fold reads: `isRunFinished(run)` (the trace finished, or a person marked it) and `effectiveOutcome(run)` (`ABANDONED` for a marked run left in progress). The fleet fold, `telemetryStats`, the safety case and drift all read it, so the four screens agree about one run; the Run Browser's tidy reaches episodes and their members.

> **Amended 2026-09-11 (WP86, `77-PIPELINE-AND-BOUNDARY.md` §3).** A seventh object store, `workflowRuns` (database version 7, keyed by `run.id`), holds `StoredWorkflowRun`s — a workflow run with the item it worked, where it came from and the run it was forked from — for the Workflows list and the Pipeline; the memory store keeps the same rows, the harness's file store `<root>/workflows/<id>/stored-workflow-run.json` beside the bare `workflow-run.json` its commands write. `Storage` gains `putWorkflowRun`, `getWorkflowRun`, `listWorkflowRuns` (newest first by the run's own start) and `deleteWorkflowRun`; *Forget everything* clears the store with the rest. Outside the run cap, as campaign reports are.

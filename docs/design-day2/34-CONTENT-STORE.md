...
> **Amended 2026-09-13 (WP109, `96-CONTROL-ROOM-V3.md` §2.2).** A fifth kind, `view` (`local/views/<slug>`): a saved Workshop view — a lens, a route and a search string, with a title (`schemas/view.ts`, `SavedView`). The envelope validates it like the others; `localPackFrom` skips it (a view is the Workshop's, never a pack's); the registry never sees one; it is not an evidence kind and never enters a kit file. The rail lists the current lens's views and saves the browser's URL as one.

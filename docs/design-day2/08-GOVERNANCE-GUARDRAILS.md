...
>
> **Amended 2026-09-12 (WP94, `85-COMPONENTS.md`).** The `Guardrail` interface is unchanged; `GuardrailComponent` in `core` is an adapter *over* it — a declaration (technique, points, verdicts, cost, connection) and a `compile(config, deps, point)` that calls the lane's own factory and stamps `componentId`/`point`. `governance` ships the adapters (`builtinComponents`, `policyCardComponent`, `guardServiceComponent`, the egress rules), `compileComponents` and `componentDepsFor`; each pack registers its components on `PackManifest.guardrailComponents`; `checkComponent` in the testkit proves each. The identity test (`85-…` §6) holds bricks and components to the same `guardrail.checked` sequence over every shipped campaign.

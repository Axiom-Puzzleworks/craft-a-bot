...
> **Amended 2026-09-12 (WP96, `85-…` §4).** `verdictForReading` writes the two component verdict kinds: a reading with `redactedText` makes any allow a `redact` (the text on the verdict, the matched `sensitive-data` finding beside it); a `note` disposition is an `annotate` with the fired finding. A block, an ask and a stop are unchanged by `redactedText`. `ArmorServiceConfig.offlineFixture` names the envelope the stand-in serves.
...
> **Amended 2026-09-02 (Stage E — WP39 closed).** The generic brick, the conformance check and the panel landed as §4.6–§4.7 describe, with four divergences worth their own letters. **D-h — the Guard Brick's service block is JSON text** (`serviceConfig: string`, parsed by the chosen service's `configSchema` at validation and at `createRuntime`), not the nested `service: <service.configSchema>` §4.6 sketched: the block's shape depends on which service is picked, which a static config schema — and so the schema panel — cannot know; the vendor-specific form is WP42's, when a second service makes the pattern real. **D-i — three `BuildProblem` codes joined core's closed union** (`guard-no-service`, `unknown-guard-service`, `guard-service-config-not-json`), all warnings, the local floor running regardless. **D-j — the schema panel's `object` case is real**, not "verify and cover": `describeFields` recurses into a nested `z.object` (through `.default`/`.prefault`, with hints looked up dotted — `'screening.screenDecision'`) and `SchemaPanel` draws it as a fieldset through a recursive snippet, patching the block back as one value; a `choice` hinted with a `source` now draws from a catalogue (`guardrailServices` is the sixth `ControlSource`). And, verified as `27-…` §7's WP52 row asked: **`ArmourPanel.svelte` never existed** — the Armour Brick's `filters` block was always the schema panel's text fallback, and is now its `object` case. `checkGuardrailService` (`pack-testkit`) runs six checks — well-formed, config parses, offline answers, `create` never throws over three broken transports, no secret leaks, egress declared — and the broken fixture pack fails each; `describeConformance` runs it for every service a manifest ships, so `pack-geap`'s contract test now covers Model Armor through it. Gate: workshop pack 8 files / 71 tests (the Guard Brick runs `starter/warning-sign` offline through `geap/model-armor` with the same verdict *shapes* at the same hooks as `geap/armor` — the wording differs by design, the decision must not); testkit 32; geap 177 with the golden trace untouched; workbench 66 files green with the schema-field tests for the two new cases; e2e 147/147 across two runs — `compare.spec.ts` and `duo-persistence.spec.ts` each failed once under the full gate running beside the unit suite and lint, and passed alone, the same load sensitivity `27-…` §8 item 1 records; lint (28 tasks) and the build green.
>
> **Against §11:** (1) both golden traces byte-identical at every stage — held, and stage D's fixture scare was prettier, not code; (2) geap's suite green and no disposition/clamp/record code under `packs/geap/src`, the wire client's own timeout and token scrub staying with the transport (stage D's note); (3) a test-only service through the shell at all three hooks and every disposition — `governance/src/hosted/*.test.ts`; (4) `checkGuardrailService` rejects the broken fixture services on every check; (5) `workshop/guard` fits with the door open, is invisible shut, and screens `warning-sign` offline with `geap/model-armor`'s verdicts; (6) every prior trace parses; (7) the next-vendor test: a pack shipping only a `GuardrailService` is fittable through `workshop/guard` with no change outside its own directory — and `pack-testkit`'s example pack is exactly such a pack.

> **Amended 2026-09-02 (Stage D).** `pack-geap` is on the shell and **`trace.geap-armour-offline.v1.json` is byte-identical** — the whole stage's gate, held. What landed: `reading.ts` gained `toScreenReading` (eight findings in trace order, `vendorLabel` the key each filter always had, `vendorConfidence` Model Armor's own string, categories by §4.5's map, `csam` as `harmful`); `service.ts` is `modelArmorService` (`geap/model-armor`, three hooks, the `geap` credential, regional egress, `alwaysStop: ['csam']`, `armorServiceConfigSchema`, `create`/`createOffline` over the unchanged wire client, `screen` choosing `sanitizeUserPrompt` at `pre-think` and `sanitizeModelResponse` otherwise — or whenever a screen carries context) plus `armorStrings`, `screeningFor`/`serviceConfigFor` (the config split; `armorConfigSchema` untouched, as the spec snapshot requires) and `armorSelectors` (`text.ts`'s history walk in the shell's shape); `brick-kind.ts`'s `createRuntime` is the local floor plus `createHostedGuardrails`; the manifest ships `guardrailServices: [modelArmorService]`. `guardrails.ts` is two adapters — `verdictFor` and `armorGuardrail` in their old signatures — so the 585-line verdict table and the record tests run unchanged as the regression net. Three notes. **`injectionMinConfidence` now does something**: it was never read by the client (Model Armor applies the template's own floor server-side), and through `screeningFor` it becomes the shell's `minConfidence`, a second, local floor on findings that carry a confidence — the only finding that does is injection, and every fixture's verdict is unchanged. **The wire client keeps its own timeout and token scrub**, against the row's literal wording: they are transport, not policy, and a second vendor's client will have its own; the *shell's* timeout is the `AbortSignal` it hands every `screen`. **`armorGuardrail` builds a rail even when the hook's dial is `off`** (reading it as `note`), because that function always did and its tests rely on it; the brick itself never asks for an off hook. Found on the way: running prettier from inside the package reformats the golden fixture the root `.prettierignore` protects — run it from the root. Gate: geap 13 files / 176 tests (ten added for the service), golden trace byte-identical, governance 164 at 100 %, every package's suite, lint (28 tasks) and the build green.

> **Amended 2026-09-02 (Stage C).** The shell landed in `packages/governance/src/hosted/` as §4.4 describes — `config.ts` (`hostedScreenConfigSchema`), `strings.ts` (`HostedStrings`, `defaultHostedStrings`), `selectors.ts` (the three defaults, reading the widened context first and walking `history` second), `verdict.ts` (`verdictForReading`, `clampForHook`), `guardrails.ts` (`createHostedGuardrails`, `filtersForRecord`) — exported from the main barrel and gated at 100 % on every axis in `vitest.config.ts`. Three notes. **`perCategory` is a `z.partialRecord`** over the category enum: a plain `z.record` with an enum key is a *total* record in Zod 4, which would have required every category to be named. **The clock is injectable** (`now`), so the shell's latency test holds it still and the golden trace's `latencyMs: 0` does not depend on the wall clock being fast. **`envelope` takes the context** rather than being a thunk, since the tick is on it. Gate: 41 shell tests over a test-only service (`test-service.ts`, excluded from coverage like `test-context.ts`) — the dial × clamp table at all three hooks, confidence against `minConfidence`, several findings folding strictest-wins, `alwaysStop` before any dial, transport and `partial`/`failure` through `onFailure`, the record in the golden key order with `outcome` offline/transport/reading, "nothing to check" making no call, `check()` dropping the record, and all three hooks through core's own `runGuardrailChain`; governance 13 files / 162 tests; lint (28 tasks) and the build green.

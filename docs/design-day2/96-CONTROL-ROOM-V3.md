...
- **The tapes' reference band**: `Tape.reference.band` as a shaded region (the interval) behind the hairline; the Monitor's rate tape passes the expected approval's Wilson interval over the window's items (`rateWithBand` from `@craftabot/metrics`).
- **The Kit's Playground box** keeps its wave-2 art: the covers are the Workshop's (the journeys page and the Playground page's strip), because the shelf's box is one file per edition on the commission's seam and a strip of eight covers is not a sticker. `83-…` §9's "the journey covers on the Playground box" is read as the Workshop's Playground page. **Divergence, noted.**
...
> **Stage B — 2026-09-13.** Richness, as §4: the roundels `catalogue` and `domain` (twenty-one; `63-…` §5.2's note; the Catalogue's title, the coverage strip), the journey covers (`covers.ts` with `DELIVERED` empty and the placeholder drawn from the layout — lanes as bands, stages as stops, the journey roundel — `JourneyCover.svelte` on the journeys page's cards and the Playground page's strip; `covers.test.ts`: one per journey, no two alike, the same twice, palette colours only), the tapes' band (`Tape.reference.band` as a region, the Monitor's expected approval with its Wilson interval; the component test), the glyph test over `STATUS`. The Kit's box untouched (§4). Budgets as the build says (`01-…` §8).
>

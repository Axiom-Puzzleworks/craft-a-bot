...
> **Amended 2026-09-06 (WP72, `61-LAST-DECKS.md` §4.3).** The bank ships two pieces of content beyond its lines and rows: the *Fallback* policy card (`fs-bank/policy/fallback`) and the *told plainly* evaluator (`fs-bank/told-plainly`) — in the bank because every desk requires it, so the three campaigns gate on one id. Still content, still no runtime. The DISP and SS1/21 rows of its control map carry their evidence and read `unreviewed`.
> **Amended 2026-09-11 (WP81, `70-CONTEXT-AND-ONTOLOGY.md` §5).** A tenth line, `fs-bank/graph` — the bank's ontology (`src/ontology.ts`: twelve classes including the governance entities, purpose-bound relations, `knowledgeCard`) as `neighbours` · `path` · `describe`, tier `observe`, under the desk's purpose; and `bankContextRecords`, the four bank desks' rungs beyond the case file. §4.5's nine are ten.
>
> **Amended 2026-09-10 (WP74 stage A, `66-CALIBRATION.md`; `64-…` §6.1.2).** §4.2's hand-set weights are rows now: every inline table and rate in the five generators is read by id from a `CalibrationTable` (`weightedRow`/`rateOf`, the same draw arithmetic), and `bankCase(seed, { calibration })` chooses which. Two ship on the manifest: `DECK_WEIGHTS` — these weights exactly, as typed assumptions, the default, so every deck and golden holds (`legacy-digests.json` proves seeds 1…200 byte-identical) — and `CALIBRATION`, the cited table the population draws from, every row `review: 'pending'` until read against its source. The distributions §4.2 chose by hand are now what they were named for: a design-time table a reviewer can see beside the cited one. **The population** (WP74 stage B, `66-…` §4.3): `population(seed, options)` — the bank as thousands of customers over a calendar, drawn from the cited table through the same generators in the same order, the transactions a lazy stream, customer *k* the same at any size, a digest as its identity; `customerCase(pop, id)` hands one to a desk in `bankCase`'s shape. The Playground's bank page shows the table and makes a population (stage C). **The books** (WP75, `67-…`): `loanBook(pop, judge)` and `alertBook(pop)` — the population as work items with their truth, the synthetic performance label on every application and the planted label on a departure, held beside the stream and never on a transaction; the alert rule, a stated detector, is the fraud journey's `rules-only` baseline.

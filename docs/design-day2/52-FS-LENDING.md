...
4. **A campaign cell's seed never reached the world** (found at stage D). `runCampaign` handed each cell's seed to the scripted brain's noise and nothing else: the starter harness built its clock without one, and an injected world was created with no `random` at all — so every cell of a campaign saw the *same* customer, and a `parity` gate had one cohort to compare (WP62's age-band gate was inconclusive, and said `passed: true`). **Fixed in `starter`'s harness and `evals`:** `RunOptions.seed` on `runToCompletion` (the clock's seed), `injectedWorld(…, random)`, and the campaign runner passing the cell's seed to both. Two seeds are two cases; the pair's two sides both appear. Stage D.
5. **Not changed, recorded:** a card cannot say "each of the decision's reasons" — a `PredicateExpr` is static data and the decision's reasons are a run's. *Reasons are real* is therefore two things: the desk's own refusal (an `explain-decision` naming a reason the decision did not give is `ok: false`, a finding on the trace) and the card guarding the order (`explain-decision` blocked before `decided`); `explanation-faithful` is the evaluator that scores it. §8.
6. **Nothing else.** Truth with a cohort, the counterpart's `knows`, `tool-result` on the bureau line, the five metrics, the plan seam, the Playground's map — as the contracts say.
...
| A matched pair "one seed apart" | The pair's side from the parity of the seed the runtime derives from its `random`; the campaign's four seeds land on both sides | A layout takes a `random`, not a seed, and a session's clock is not `seededRandom`; the finances are a template either way |
...
7. The PR description names the contract changes: `prompt-contains`, `ledger.loans`/`ledger.appeals`, `reasonsUsed`, and the campaign cell's seed reaching the world.
...
> **Stage D landed 2026-09-06 — WP63 closed.** `campaigns/fs-lending-baseline.json` from `src/campaign.ts` (§4.6), the file regenerated and compared by its test: sixteen scenarios × the desk bot (the bureau line on a Connector) × four guards × two brains × four seeds. The gates: the optimal plan wins everywhere; under `none` the adversary decides the clear approve unverified and the payout asks no one; under every card guard `agree` ≥ 0.95 and `missed-refer` = 0 on the optimal brain, `explanation-faithful` = 1, `appeal-handled` = 1, `identity-before-decision` = 1 on *either* brain (the card stops the early decision), and the clear approve's payout asks a person first (`approvalsRequested` ≥ 1 — an assertion card cannot say "without approval", so the metric does); **the Playground's first matched `parity` gate** — the pair's agreement across the proxy, `maxDifference: 0`, `matched: true`, the verdict carrying both sides at 1 — beside an unmatched one over over-decline across age bands. **The red run** as a permanent test: *No decision before affordability* removed → `identity-before-decision` fails under the cards. **The planted skew** as a permanent test (§11 item 3): a test-only reflex brick that declines the older side of the pair on the age band the application *does* reveal, fitted beside the cards as a fifth guard — the matched gate fails with the two sides at 1 and 0, and every skewed pair cell is `over-decline`. Two things the campaign taught the note: (1) §2 item 4 — the cell's seed never reached the world, so the pair had one side and WP62's parity gate had been inconclusive; fixed in the starter harness and `evals` before this test could mean anything; (2) with nothing decided in a run, `decision-matches-rules` scores the decision already on the file (the appeal layout's, now one that agrees with the rule), so an `agree` rate over a deck with appeals in it is honest. CI runs the file as the campaign job's fourth step, its SARIF under its own category. The Kit: `lending-desk.spec.ts` — the card behind the Workshop door, then the application in the queue pane with the file to earn. `/workshop/playground/lending` (§4.7) on the Playground's page pattern: a layout and a seed, the application on the desk and the worksheet, the payslip and the statement on file, the verdict and the cohort under the flap, the decks, the five cards, the five evaluators, the campaign build on the map with the bureau line outside. Close-out: `42-…` §3 and §8 items 34–37 (Phase N closed), `41-…` §12, `CLAUDE.md`, `README.md`, `docs/playground.md`, the shelf box, the harness README. Gate: root lint, every workspace's tests (the pack at 86), the build, the evals baseline, the default e2e and the visual set.

> **Amended 2026-09-06 (WP64, `56-LIVE-COUNTERPARTS.md` §4.3).** The baseline gains a fifth guard, `compliance-watchbot` — the cards, a Monitor Judge per conduct evaluator noting every tick, the Watchbot, and at the chokepoint the breaker on `identity-before-decision` failing — the Lending Desk's Compliance Watchbot stack, one more column of every table with the gates unchanged. On this desk a live counterpart (`craftabot run --counterpart`, a live-seat campaign) is the case's own person, generated with the case and read from the world the host makes (`56-…` §2 item 10).

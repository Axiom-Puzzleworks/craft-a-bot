...
> **Amended 2026-09-12 (WP102, `94-HANDOFFS-AND-COMPLAINTS.md`; `83-…` §6.5.3).** `StageSpec.next` may return a handoff — `{ handoff: workflowId, item: WorkItem }` — and the run ends there with `outcome: 'handed-off'` and `WorkflowRun.handoff { to, itemId, item }`; the target run, started by the host through `followHandoff`, carries the chain back on `handoffs[]`. The item, never the desk state. The book cell records the handoff and counts the run a success; the clock puts the item back on the day as an arrival; `craftabot workflow run --follow` runs the chain. The `BankRun` counts gain `handedOff`. No new event: a workflow run's own events are the `stage.*` pair, and the outcome is on the record.

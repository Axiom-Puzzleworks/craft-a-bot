...
> **Amended 2026-09-02 (stage C — WP48 closed; Phase K closes with WP49, not here — corrected 2026-09-03).** Gate: core (+3), monitor (+2), starter (+2), harness (+1), workbench (+2); lint, build within budget, e2e green. `23-…` §4.7's WP34 amendment and §9's Hearing row point here. `27-…` §8 item 17 carries the summary.

> **Amended 2026-09-06 (WP64, `56-LIVE-COUNTERPARTS.md` §4.3).** Beside the breaker on refusals, `createEvaluatorCircuitBreaker(evaluator, { labels?, onFail? })` — the desk's own evaluator over the merged history at the group's `pre-think`, `stop-run` on a named label or a fail — and `createComplianceWatchbot({ watchFor, refusalLimit?, breakOn })`, the group Watchbot with the breakers beside it: `41-…` §6.5.6's monitor, named as content by every desk campaign's `compliance-watchbot` guard and installed by a two-seat cell or `craftabot run --counterpart … --stack`. Truth never reaches a chokepoint (`56-…` §2 item 11), so the breakers fire on what the trace alone can judge. `CreateSessionGroupDeps.world?` lets a host hand the group a made world.

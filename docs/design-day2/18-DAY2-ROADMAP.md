...
3. **WP16 (P0 UX) — in progress.** Slices a, b, c and d are on `main`; **only slice e remains** (§1.4, run continuity — persistence, the Scrapbook and the replay viewer, and the largest of the five). Finishing it closes everything in WP16 that a coding session can close; the two human checks in its DoD cannot be done from here. WP19's harness skeleton can still start in parallel. Note that WP15 (strategy seams E7) was **skipped**, not completed — see its row in §3 before assuming Phase A is closed.

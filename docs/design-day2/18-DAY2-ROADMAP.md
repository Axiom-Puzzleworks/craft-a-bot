...
| WP17 | **P1 UX wave** (16 §2: safety centre-stage, tutorial gap-fixes, celebration/identity, naming chips, free play real, hearing input, a11y completion) | 🚧 **In progress — slice a landed 2026-08-14.** Sliced five ways: **a** a11y completion + the axe harness (16 §2.7) ✅; **b** free play made real + hearing (§2.5, §2.6) ⬜; **c** safety centre stage (§2.1) ⬜; **d** celebration/identity + naming chips (§2.3, §2.4) ⬜; **e** tutorial gap-fixes (§2.2) ⬜ — **last deliberately**, since the tutorial must teach what b–d build. Slice a's axe pass exposed an app-wide contrast fault (`04-…` §2.3); the remaining DoD is 16 §2 acceptance tests |

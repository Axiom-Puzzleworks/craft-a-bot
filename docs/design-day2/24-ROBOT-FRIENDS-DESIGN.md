...
> **Amended 2026-08-20 (WP31 stage C):** §4.4's `subjectFor` sketch — swapping a fixed "You"/"It" inside a caption's own words — is not what shipped. The reason is `beatFor`'s `action.performed` case: **the caption most players will ever actually see there is not synthesised at all**. When a pack supplies a non-empty `narration` string (`"You roll one square north."`, written once, by hand, in the pack's own second-person voice), that string is used verbatim — the `"It tried to {name}"` fallback `subjectFor` was written for only fires when a pack forgot to write one, which no starter-pack action ever does. A `subjectFor` that only ever rewrites unused fallback text would not satisfy the stage's own acceptance goal ("both robots narrated by name") for a single real gameplay beat.
>
> What shipped instead: `Beat` gains an optional `actor?: string` field, resolved once per event from `actors.get(event.agentId) ?? 'A robot'` (same fallback, same "omit `actors`, get `undefined` everywhere, byte-identical output" guarantee as the sketch). No caption's words are touched, ever — not the synthesised fallbacks, not a pack's own narration. `StoryStrip` renders `actor` as a small label ahead of the caption ("**Robo:** You roll one square north.") in the beat bubble, the past-turn tooltip, and the live-region/read-aloud sentence. This is uniform across every beat kind with an `agentId` — including ones §4.4 never named, like `heard` (input delivered to a specific member) and `result` (whose approval got resolved) — rather than requiring a per-case decision about which pronoun each caption happened to use. `narrate.test.ts`'s own gate — every pre-existing case byte-identical — holds for the same reason it held for the sketch: `actor` is a new optional field nothing before this stage reads.

> **Amended 2026-08-20 (WP31 stage D):** three implementation facts found while wiring live persistence and replay, neither of the first two changing §4.5's shape of what gets stored, the third a real core-adjacent bug this stage's own persistence work is what finally exposed.
>
> - **Replay does not fold a group's merged trace through the bare `projectThrough`, unchanged.** §4.5 says the replay renders "through the one fold, unchanged", read most naturally as "call `projectThrough` on the merged stream, same as solo." That call would have compiled and mostly looked right — `run-projection.ts`'s fold has no `agentId` awareness, so a raw two-agent merged stream cross-wires both robots' history into one set of fields. Round-robin block-ordering hides most of this by coincidence (each round's events arrive as two whole blocks, so whichever member acted last is naturally who should be foregrounded), but sticky fields do not reset between members: a `tripped` flag set by one robot's guardrail trip would still read `true` on a later round foregrounding the *other* robot, who never tripped anything. Fixed with a new pure fold, `group-replay-projection.ts`'s `projectGroupThrough`: it finds which member's own turn produced the world as of the scrubbed tick (from the merged stream's own `world.changed` events, which *are* safe to fold raw — every payload already carries the whole shared room), then refolds *that member's own events only* through the existing, unmodified `projectThrough` — recovering the single-session subsequence it was built for. `run-projection.ts` itself is untouched, exactly as `23-…` principle 2 asks. Proven directly: a hand-built trace where one member trips a guardrail and the other never does, asserting the second member's own replay reads `tripped: false` (`group-replay-projection.test.ts`).
> - **Group episodes are excluded from `evictOldRuns`'s cap entirely, not capped by their own scheme.** `23-…` §8 deferred group-aware eviction pending "a live producer" — this stage is that producer, so the deferral's own condition is now false. Rather than design the real retention scheme in the same stage that first makes it needed, `selectRunsToEvict` now excludes any run carrying a `groupRunId` from candidacy: evicting one member's own row alone would corrupt its still-live `GroupRunRecord` (a `memberRunIds` entry pointing at a 404), which single-run eviction was never built to avoid. Conservative on purpose — a grouped run currently never ages out — recorded in `23-…` §8 as well, since it is that file's own deferred decision being revisited.
> - **`GroupSessionView.round` fixed to update during PLAY, not only when `stepRound()` is called directly.** Found live: a played-to-completion duo episode's own `GroupRunRecord.rounds` read `0`, because `session-group.svelte.ts`'s `stepRound()` wrapper was the only place `state.round` was ever set, and PLAY mode drives rounds through the core's own `groupPlayLoop()` — `SessionGroup.stepRound()` called directly, never through this view's wrapper. Stage B's PLAY re-callability fix made that path fully reachable without ever exercising the one field that read it. Fixed by tracking `state.round` off every absorbed event's own `tick` instead (`state.round = Math.max(state.round, event.tick)`, in effect) — the same "round-robin block-ordering means tick only grows" property `projectGroupThrough` above already leans on — so it now updates correctly regardless of which path drove the group. `session-group.svelte.test.ts` gained a regression test that drives a group to completion purely through `start('play')` and asserts `view.round` lands on the right value; all pre-existing tests, including the ones that only ever call `stepRound()` directly, are unaffected.

> **Amended 2026-08-20 (WP31 stage E):** two implementation facts found while building the picker.
>
> - **"GO-ready" for a candidate partner means both of `benchStore.canGo`'s own checks, not only the one §4.6 names.** The sentence reads "GO-ready per the existing `validateSpec(candidate.spec, registry)` blocking-problem check", naming just one of the two conditions that actually gate the solo `GoLever` (`disabled={!benchStore.canGo || batteryMissing}` — `validateSpec`'s blocking problems, plus the battery/cartridge check `validateSpec` cannot see, since keys never touch the spec). Offering a candidate whose own battery is missing would let a player pick a partner whose launch then immediately fails in `/play/duo`'s own load error — worse than not listing it. The picker's `isGoReady` checks both, mirroring `benchStore.canGo`'s exact two-part definition rather than the doc's shorthand for it.
> - **The Robot Friends lever is neutral (cream/ink), not a coloured fill.** `04-VISUAL-DESIGN-LANGUAGE.md` §2.2 fixes blue to LLM ("**Never reassign these colours**"), so a first draft that gave the lever a blue fill (a plausible "secondary action" choice with no other precedent to check against) would have broken the one hard rule this doc's own visual work has to answer to. Caught before shipping, not after; the lever borrows the bench's own existing secondary-control style (the Undo button) instead of inventing a new one.
>
> Also found, in `apps/workbench/e2e/support.ts`'s `buildReadyBot` (test infrastructure, not app code, so not really a "divergence" but recorded for the same reason the eviction and `view.round` fixes are): `benchStore`'s own saves are debounced 250ms, and nothing in the DOM signals when one has landed. Two `buildReadyBot` calls in a row — every duo e2e spec's own pattern since stage B — race a first bot's last edit against a full-page navigation to build the second, which abandons the pending save outright. It had not been hit before this stage's own tests reopened a *previously-built* bot's bench (`goto('/bench/{id}')` after building a second one) rather than only ever reading straight through to `/play/duo`; `buildReadyBot` now waits out the debounce window before returning.

> **Amended 2026-08-20 (WP31 stage F):** §4.7's own claim — "nothing here is a new *mechanism* (hard rule 4)" — was wrong, and building the Radio brick is what found the gap.
>
> - **A brick's `channel`/`allowFrom` config has to reach the *world*, and the open brick contract had no door for that.** `AgentHandle` (`types/world.ts`) is deliberately identity-only — "a world learns *who* is asking; *what bricks they have fitted* stays entirely inside the session" — so Radio's send/receive can't read its own config off the handle without breaking that boundary. The first idea, parameterising the sense-channel id itself (something like `starter/playroom/radio/work`), is what `validateSpec`'s `senseProblems()` rules out: registered sense-channel ids are a static, pre-registered set (`registry.getSenseChannel(channelId)`), and an arbitrary builder-chosen channel value can't be one of them. `contributeContext` was no help either — it only ever sees `{tick, channels}`, never the raw sensed data, so it has nothing to filter.
> - **What shipped**: a small, symmetric extension to the same optional-hook pattern `contributeCalls`/`contributeSenses` already use. `BrickRuntime` gains `contributeWorldConfig?(): Record<string, unknown>` — a brick hands back a bag of config keyed by whichever sense-channel or action id it concerns (Radio keys its bag by `qualifyPlayroomId('radio')`). A new `collectWorldConfig()` in `brick-runtimes.ts` merges every fitted brick's bag (`Object.assign`, later brick wins on a key collision — a build question, not something to arbitrate here, exactly as `collectCalls`/`collectSenses` already treat duplicates). `WorldInstance` gains `configure?(config): void`, called once by `createSession`, right after the runtimes are built and before the first tick. `AgentHandle` itself is untouched, every existing `WorldInstance`/`BrickRuntime` is unaffected (both hooks optional), and it needs no change at all to `session-group.ts` — `createSession` already runs once per group member with that member's own spec, so `collectWorldConfig(runtimes)` is naturally the right bag for that one member already. `02-AGENT-MODEL.md`'s own dated amendment records the same mechanism from the engine side.
> - **Radio's presentation lives on its `BrickKindDefinition` directly** (`name`/`description`/`realName`/`realExplanation` inline), not through `bricks.ts`/`facesOf()` — the indirection that exists only because the six V1 bricks had presentation data before the open contract existed. This is the same choice Watchbot made when it joined post-contract; `bricks.ts`'s own header comment now says so, and `brick-kinds.test.ts`'s parity check iterates `starterBricks` (the bricks old enough to have a second copy) rather than every kind, so Radio having no second copy to drift from isn't mistaken for a gap.
> - **`describeFitted` can't name `allowFrom`'s trusted senders, only count them.** It receives just the brick's own config — no registry, no other agents' records — so `['<uuid>']` can't become "trusting Bolt." The tray copy says "trusting 1 other robot" instead. Deliberate and honest, not a TODO: the alternative (reaching into a group's member list from inside a slot definition) would be the exact kind of mechanism-in-a-pack this rule exists to prevent.
> - **A genuine, pre-existing e2e gap, surfaced by Radio's own tray entry**: `PartsTray.svelte` has no `overflow`/`max-height` — the tray simply grows as the catalogue grows, and Radio's addition (an eighth item, between Tools and Sense) was enough to push `tray-starter/sense` below a standard test viewport. `build-mouse.spec.ts`'s `dragBrickToSocket` computed bounding boxes once and drove raw `page.mouse.move` between them, which silently fails once either endpoint sits outside the viewport (unlike `.click()`, Playwright does not auto-scroll for coordinate-based pointer moves). Fixed at the test-infrastructure layer — `scrollIntoViewIfNeeded()` on the source immediately before reading its box, and again on the destination immediately before the final move — rather than at the production layer: an independently-scrollable tray is a real UX question worth its own decision once Planner/Librarian/Connector keep growing the catalogue further (`18-…`'s own Phase F/later rows), not one to absorb piecemeal into a brick-adding stage.

> **Amended 2026-08-20 (WP31 stage G):** §4.8's own open question — policy card or scripted contrast alone — is answered, and it did not need deciding from scratch.
>
> - **No new policy card.** `starter/policy/no-secrets-out-loud` (WP25) already reads "Blocks saying the cupboard code, however the bot was talked into it" — written before this scenario existed, and already general enough to cover it. A `pre-act` rule keyed on `call-name-is: 'say'` and `argument-equals: LEAK_PHRASE` does not care what convinced the bot to try; reusing it outright, rather than writing a Radio-aware rule that duplicates the same block, is the more honest read of "however."
> - **`starter/party-line` reuses `keep-the-secret`'s exact lethal-trifecta shape** — same layout geometry, same `hello-said-secret-kept` predicate, same `LEAK_PHRASE` — and changes only the *request* leg's delivery, from a sign taped to the wall to a message claiming a false identity on the Hearing channel. This is also what keeps the card solo-winnable: `solvability.test.ts` requires every card (`coop` ones included, following `tidy-together`'s own precedent) to have a scripted-optimal plan a lone bot can run to `SUCCESS` inside the default budget, and the safe solve here is `plans.ts`'s existing `IGNORE_THE_SIGN` — unmodified, because ignoring untrusted content was already the optimal play regardless of which channel it arrived on.
> - **The spoofed claim is seeded content, not a scripted mid-run event.** §4.8's "partway through" reads as timing, but nothing in the harness (`runToCompletion`, `runGroupToCompletion`) exposes `WorldInstance.receiveInput` as a scriptable step — that method exists only for a live person's Free Play input. Seeding the message directly into the `party-line` layout's initial `heard` array is mechanically identical to how `manual.ts`'s own poisoned entries sit there from the start, waiting to be found: a bot with no ears fitted never sees it, same as a bot that never opens the manual never meets the sign. No new mechanism, no new harness plumbing.
> - **The message names no specific teammate.** A player, not the design, chooses what their two robots are called, and a static layout string cannot know it. `"it's your teammate!"` rather than `"it's Bolt!"` sidesteps the mismatch entirely — and reads as a more honest example of the real attack besides: a spoofed message asserting a vague, plausible identity rather than one it could get wrong and be caught on the name alone.
> - **The ASI07 trace property (§11.7) needed a specific seat order to demonstrate, not new engine behaviour.** `Hearing`'s `{ lines }` shape has never carried a sender, and `Radio`'s `{ messages: [{ from, fromName, ... }] }` shape always has (`23-…` §4.8's seat-swap attribution, `24-…` §4.7) — the contrast was already true by construction. Putting it in one `sense` event for a test to check needed Bolt scheduled *before* Robo in the `SessionGroup`'s member order, so Bolt's real message lands on the shared log — and Hearing stays undrained — before Robo's own first turn observes both. `session/party-line.test.ts` is the proof, over a real two-seat group; `solvability.test.ts`'s solo plan cannot show this contrast at all, since there is no second seat to attribute against.
...
**Checked at H, 2026-08-20 — all seven hold.** (1) `session/party-line.test.ts`'s undefended run (leaks, fails the card) and defended run (policy card blocks it, card succeeds), plus `robot-friends.spec.ts`'s full e2e walk, shelf → bench → Robot Friends → duo Playroom → scrapbook. (2) Every one of the eight stages landed on its own full-gate pass — unit suites, `check`, `lint`, build+budget, all e2e — before the next stage began; none was ever built on a red prior stage. (3) Proven from several directions rather than one dedicated fixture: `brick-matrix.test.ts`'s Radio-off tests show a bot with no Radio brick fitted is never offered `radio_send` and never carries a `radio` sense channel; `party-line`'s coop card is excluded from the Kit's own goal-card rack by the same `!card.coop` filter WP29 built (`bench/[agentId]/+page.svelte`, unmodified this WP); and every pre-existing test in both packages stayed green throughout, with every touched assertion a genuine widening (an action/sense/brick/layout *count* growing by one) rather than a changed expectation. Robot Friends only ever *reads* a shelf bot's spec to build a duo session (`createGroupSessionView`, unchanged since WP29 stage A) — it has no write path back to a shelf bot at all, so "no shelf bot mutated" is true by the absence of a mechanism that could do it, not by a check that happens not to catch one. (4) `narrate.test.ts`'s byte-identical assertion on every pre-existing case, plus `duo-play.spec.ts`'s e2e narrating both robots by name. (5) `duo-persistence.spec.ts`'s e2e: a live-played duo run turns up in both the Workshop Run Browser and the Kit Scrapbook, and both replay. (6) `radio.test.ts`'s per-recipient cursor test (a message is delivered exactly once, regardless of which seat acts first) and its `allowFrom` tests (an unlisted sender is filtered, a listed one is not). (7) `party-line.test.ts`'s own attribution-contrast test: the spoofed claim and Bolt's real Radio message sit in the same `sense` event, and only `Hearing`'s `{ lines }` shape carries no sender while `Radio`'s carries `from`/`fromName` — mechanical, never narrated at the player.

> **Amended 2026-09-06 (WP64, `56-LIVE-COUNTERPARTS.md` §4.4).** On a desk card the second robot's seat is *live* whenever its brain is a real provider — keyed, or keyless like Ollama — and *scripted* only for the demo brain (until now a keyless provider was taken for the demo and the script played instead, `56-…` §2 item 12); the roles line says which, and names the cartridge. The Robot Friends picker names who sits across a desk card.

...
> **Amended 2026-09-11 (WP87, `78-LENSES.md` §4).** `/workshop/assurance` is also the Assurance lens's entry: a Strip at its head with the safety case's four claims as Lamps (*inability stated*, *reach named*, *guardrails installed*, *runs finished*), incidents and drift flags over the selected bot, and the pack's download; beneath it the Control Effectiveness Register as an empty `CaseTable` that says *untested* until WP90 folds one; and two stored reports into Compare. The pack's own rows and the three downloads are unchanged.

> **Amended 2026-09-13 (WP110, `97-ACCESS.md` §1, decision 4; GAP-1).** A filed control row may carry a reader's **review** (`AssuranceControlRow.review`: status, by, note, reviewedAt) from `AssurancePackInput.controlReviews` — the `control-review` content records the Workshop's store holds, read by `assurancePackFromStorage`. Both renderers add a *Review* column. The row's own `status` (`unreviewed` / `pending` / `reviewed`) is the pack's claim and stays as the pack made it; the review is the reader's, beside it.

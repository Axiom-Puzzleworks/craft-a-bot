...
> **Stage C landed 2026-09-05 — WP55 closed.** **The harness:** `craftabot run --counterpart scripted|live [--counterpart-cartridge <id>] [--max-rounds <n>]` (`commands/run-duo.ts`) seats a generated visitor — `say` and `hang-up`, the conversation and its brief, the script's persona as its personality, its ids **qualified with the desk's own** because the starter's bricks qualify a bare id with the Playroom's (`12-…` D20) — beside the kit's bot in a `SessionGroup`, writes each member's run, the group's record and merged stream the way the Kit's duo does, and a `<groupRunId>.craftabot-bundle.json`; a room refuses it. The scripted brains now read `@craftabot/pack-workshop/testing` (new: the pack's plans, the starter's precedent) after the starter's, so a desk card plays scripted at all — until now `craftabot run` refused every Workshop card. Proven in `run-counterpart.test.ts`: the bundle's two runs and `memberRoles`, the visitor's lines under the script's name, the store's group and members, the merged stream reproduced from the seed with a pinned clock and ids, the refusal on a room. **The Kit's duo:** on a desk card the second robot is the visitor by position, driven along the desk's script through `scriptedCounterpart` when it is keyless (the Demo Brain would otherwise play the clerk's plan from the visitor's chair) and by its own cartridge when it has a battery; the route says who is who (`duo-roles`); `GroupSessionMemberDeps.role` carries it through. **The Run Lab:** a group episode draws the `agent` seat's Boundary, found through `group.started.memberRoles` (the first member when no seat had a role), with the other seats named by the host — `boundaryMapFor` gained `options.names`, since the trace carries no display names and the host's run records do — and given their roles; `two-seat-transcript.test.ts` holds over the two-seat golden that `projectThrough` and `projectGroupThrough` both fold to a transcript with both voices and that scrubbing to tick 0 leaves the opening alone. **e2e:** two desk-built robots on the Front Desk through the Kit's duo: the roles line, both voices, `duo-finished` and saved; the stored episode in the Run Lab with both speakers and the visitor as a lit node inside the clerk's Boundary. One thing seen on the way and left as it is: a live seat's brain sees only what was *said*, so a rule cued by the clerk's *action* (`signed-in` → `end-conversation`) does not fire for a live seat the way it does for the script inside the desk — the visitor thanks the clerk when the desk's own script runs and keeps talking when a seat runs it; a seat that should react to an action needs a rule on what the desk *says* about it (the system line), which WP58's service lines will want anyway. **WP55's DoD, checked:** `advanceCounterpart` table-tested; the two-seat golden byte-identical and the three earlier goldens unchanged; the group test that a live seat's `say` becomes the agent's next observation; a scripted-tier episode reproduced from a seed (the golden, and the harness twice); the Run Lab's two-seat transcript from a stored episode (unit and e2e); `checkDesk` proving every script has a `fallback` and every rule a known `when`; the tenet-13 property still green on the agent seat of both desks. G23 retired. Gate: root lint clean, every suite green (22 workspaces), build within budget with the schema check, default e2e 168/168 (one new spec), visual 3/3, baseline campaign with no regressions.

> **Amended 2026-09-06 (WP64, `56-LIVE-COUNTERPARTS.md`).** `counterpartScriptFor` and `counterpartSpec` moved from the harness to `@craftabot/evals` (`counterpart-seat.ts`), and read the script *seated* in a desk instance (`DeskWorldInstance.seatedCounterpart`) when given one — so the bank's desks, whose person is generated with the case, seat a live counterpart too; a group takes a host-made world through `CreateSessionGroupDeps.world`. A campaign names a live seat (`counterpart.tier`), and the Compliance Watchbot sits at an episode's chokepoint.
